...
public class Server {
    ...
    public static void main(final String[] args) throws InterruptedException, URISyntaxException {
        ...
        final ObservableList<String> lastFilesOpened =
                JabRefPreferences.getInstance().getGuiPreferences().getLastFilesOpened();
        ...
        // This is a quick solution. The architectural fine solution would use some http context or other @Inject_ed
        // variables in org.jabref.http.server.LibraryResource
        if (args.length > 0) {
        ...
        if (lastFilesOpened.isEmpty()) {
            ...
            Path bibPath = Path.of("src/main/resources/org/jabref/http/server/http-server-demo.bib")
                    .toAbsolutePath();
    ...
    private static void startServer() {
        ...
        SeBootstrap.start(Application.class, configuration).thenAccept(instance -> {
            ...
            instance.stopOnShutdown(stopResult -> System.out.printf(
                    "Stop result: %s [Native stop result: %s].%n", stopResult, stopResult.unwrap(Object.class)));
            final URI uri = instance.configuration().baseUri();
            System.out.printf(
                    "Instance %s running at %s [Native handle: %s].%n", instance, uri, instance.unwrap(Object.class));
    ...
    private static SSLContext getSslContext() {
        ...
        Path serverKeyStore = Path.of(
                        AppDirsFactory.getInstance().getUserDataDir(OS.APP_DIR_APP_NAME, "ssl", OS.APP_DIR_APP_AUTHOR))
                .resolve("server.p12");
