...
public final class SearchResult {
    ...
    public List<String> getContentResultStringsHtml() {
        if (contentResultStringsHtml == null) {
            return contentResultStringsHtml =
                    getHighlighterFragments(highlighter, SearchFieldConstants.CONTENT, pageContent);
        }
        return contentResultStringsHtml;
    }
    ...
    public List<String> getAnnotationsResultStringsHtml() {
        if (annotationsResultStringsHtml == null) {
            annotationsResultStringsHtml =
                    getHighlighterFragments(
                            highlighter, SearchFieldConstants.ANNOTATIONS, annotation);
        }
    ...
    private static List<String> getHighlighterFragments(
            Highlighter highlighter, SearchFieldConstants field, String content) {
        try (TokenStream contentStream =
                SearchFieldConstants.LINKED_FILES_ANALYZER.tokenStream(field.toString(), content)) {
            TextFragment[] frags =
                    highlighter.getBestTextFragments(contentStream, content, true, 10);
