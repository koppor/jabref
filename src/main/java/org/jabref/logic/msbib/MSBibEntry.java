...
class MSBibEntry {
    ...
    private final Pattern ADDRESS_PATTERN =
            Pattern.compile("\\b(\\w+)\\s?[,]?\\s?(\\w*)\\s?[,]?\\s?(\\w*)\\b");
    ...
    private void populateFromXml(Element entry) {
        ...
        Optional<Date> parsedDateAcessed =
                Date.parse(
    ...
    public Element getEntryDom(Document document) {
        Element rootNode =
                document.createElementNS(MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + "Source");

        for (Map.Entry<String, String> entry : fields.entrySet()) {
            addField(document, rootNode, entry.getKey(), entry.getValue());
        }

        Optional.ofNullable(dateAccessed)
                .ifPresent(field -> addDateAcessedFields(document, rootNode));

        Element allAuthors =
                document.createElementNS(MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + "Author");
    ...
    private void addField(Document document, Element parent, String name, String value) {
        ...
        Element elem =
                document.createElementNS(MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + name);
    ...
    private void addAuthor(
            Document document, Element allAuthors, String entryName, List<MsBibAuthor> authorsLst) {
        if (authorsLst == null) {
            return;
        }
        Element authorTop =
                document.createElementNS(MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + entryName);
        ...
        if (personName.isPresent()) {
            ...
            Element corporate =
                    document.createElementNS(
                            MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + "Corporate");
            corporate.setTextContent(person.getLastName());
            authorTop.appendChild(corporate);
        } else {
            Element nameList =
                    document.createElementNS(
                            MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + "NameList");
            for (MsBibAuthor name : authorsLst) {
                Element person =
                        document.createElementNS(
                                MSBibDatabase.NAMESPACE, MSBibDatabase.PREFIX + "Person");
