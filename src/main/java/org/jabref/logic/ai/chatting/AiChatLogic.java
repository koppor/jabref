...
public class AiChatLogic {
    ...
    private void setupListeningToPreferencesChanges() {
        aiPreferences
                .instructionProperty()
                .addListener(obs -> setSystemMessage(aiPreferences.getInstruction()));
        aiPreferences
    ...
    private void rebuildChain() {
        ...
        if (linkedFiles.isEmpty()) {
            ...
            filter =
                    MetadataFilterBuilder.metadataKey(FileEmbeddingsManager.LINK_METADATA_KEY)
    ...
    private String augmentSystemMessage(String systemMessage) {
        String entriesInfo =
                entries.stream()
                        .map(CanonicalBibEntry::getCanonicalRepresentation)
    ...
    public AiMessage execute(UserMessage message) {
        ...
        LOGGER.debug(
                "Message was answered by the AI provider for {}: {}", name.get(), result.text());
