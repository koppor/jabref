...
public class MVStoreChatHistoryStorage extends MVStoreBase implements ChatHistoryStorage {
    ...
    @Override
    public void storeMessagesForEntry(
    ...
    @Override
    public void storeMessagesForGroup(
            Path bibDatabasePath, String name, List<ChatMessage> messages) {
    ...
    private List<ChatMessage> loadMessagesFromMap(Map<Integer, ChatHistoryRecord> map) {
        return map
                .entrySet()
                // We need to check all keys, because upon deletion, there can be "holes" in the
                // integer.
                .stream()
    ...
    private void storeMessagesForMap(
            Map<Integer, ChatHistoryRecord> map, List<ChatMessage> messages) {
        map.clear();
    ...
    private Map<Integer, ChatHistoryRecord> getMapForEntry(
            Path bibDatabasePath, String citationKey) {
