...
public class IngestionService {
    ...
    private final TreeMap<LinkedFile, ProcessingInfo<LinkedFile, Void>> ingestionStatusMap =
    ...
    private class EntriesChangedListener {
                            ...
                            entry -> {
                                if (aiPreferences.getAutoGenerateEmbeddings()) {
                                    entry.getFiles()
                                            .forEach(
                                                    linkedFile ->
                                                            ingest(linkedFile, bibDatabaseContext));
                                }
        ...
        public void listen(FieldChangedEvent e) {
            if (e.getField() == StandardField.FILE && aiPreferences.getAutoGenerateEmbeddings()) {
                e.getBibEntry()
                        .getFiles()
                        .forEach(linkedFile -> ingest(linkedFile, bibDatabaseContext));
            }
        }
    }
