 ...
 */
public class MVStoreFullyIngestedDocumentsTracker extends MVStoreBase
        implements FullyIngestedDocumentsTracker {
    private static final String INGESTED_MAP_NAME = "ingested";

    // This map stores the ingested documents. The key is LinkedDocument.getLink(), and the value is
    // the modification time in seconds.
    // If an entry is present, then it means the document was ingested. Otherwise, document was not
    // ingested.
    // The reason why we need to track ingested documents is because we cannot use
    // AiEmbeddingsManager and see if there are
    // any embeddings because when we ingest a document embeddings are generated in series, so if 1
