...
public class BracketedPattern {
     ...
     */
    private static final Pattern DEPARTMENTS =
            Pattern.compile("^d[ei]p.*", Pattern.CASE_INSENSITIVE);

    private static final Pattern WHITESPACE = Pattern.compile("\\p{javaWhitespace}");

    private static final RemoveEnclosingBracesFormatter ENCLOSING_BRACES_FORMATTER =
    ...
    public static String expandBrackets(
            String pattern, Character keywordDelimiter, BibEntry entry, BibDatabase database) {
    ...
    public static Function<String, String> expandBracketContent(
            Character keywordDelimiter, BibEntry entry, BibDatabase database) {
        return (String bracket) -> {
            List<String> fieldParts = parseFieldAndModifiers(bracket);
            // check whether there is a modifier on the end such as
            // ":lower":
            String expandedPattern =
                    getFieldValue(entry, fieldParts.getFirst(), keywordDelimiter, database);
     ...
     */
    public static String expandBrackets(
            String pattern, Function<String, String> bracketContentHandler) {
        Objects.requireNonNull(pattern);
    ...
    public static String getFieldValue(
            BibEntry entry, String pattern, Character keywordDelimiter, BibDatabase database) {
        try {
            if (pattern.startsWith("auth") || pattern.startsWith("pureauth")) {
                // result the author
                String unparsedAuthors =
                        entry.getResolvedFieldOrAlias(StandardField.AUTHOR, database).orElse("");

                if (pattern.startsWith("pure")) {
                    // "pure" is used in the context of authors to resolve to authors only and not
                    // fallback to editors
                    // The other functionality of the pattern "ForeIni", ... is the same
                    // Thus, remove the "pure" prefix so the remaining code in this section
                    // functions correctly
                    //
                    ...
                    unparsedAuthors =
                            entry.getResolvedFieldOrAlias(StandardField.EDITOR, database)
                                    .orElse("");
                }
                ...
                if (pattern.matches("authIni[\\d]+")) {
                    ...
                    return authNofMth(
                            authorList, Integer.parseInt(nums[0]), Integer.parseInt(nums[1]));
                ...
                if (pattern.matches("edtrIni[\\d]+")) {
                    ...
                    String[] nums = pattern.substring(4).split("_");
                    return authNofMth(
                            editorList, Integer.parseInt(nums[0]), Integer.parseInt(nums[1]));
                ...
                return getTitleWords(
                        3,
                        removeSmallWords(
                                entry.getResolvedFieldOrAlias(StandardField.TITLE, database)
                                        .orElse("")));
                ...
                String yearString =
                        entry.getResolvedFieldOrAlias(StandardField.YEAR, database).orElse("");
                ...
                return entry.getResolvedFieldOrAlias(InternalField.TYPE_HEADER, database)
                        .orElse("");
            } else if (pattern.matches("keyword\\d+")) {
                // according to LabelPattern.php, it returns keyword number n
                int num = Integer.parseInt(pattern.substring(7));
                KeywordList separatedKeywords =
                        entry.getResolvedKeywords(keywordDelimiter, database);
    ...
    private static AuthorList createAuthorList(String unparsedAuthors) {
        return AuthorList.parse(unparsedAuthors).getAuthors().stream()
                .map(
                        author -> {
                            // If the author is an institution, use an institution key instead of
                            // the full name
                            String lastName =
    ...
    private static boolean isInstitution(Author author) {
        return author.getGivenName().isEmpty()
    ...
    static String applyModifiers(
        ...
        for (int j = offset; j < parts.size(); j++) {
            ...
            if ("abbr".equals(modifier)) {
                ...
                String[] words =
                        resultingLabel.replaceAll("[\\{\\}']", "").split("[\\(\\) \r\n\"]");
                ...
                if (formatter.isPresent()) {
                    resultingLabel = formatter.get().format(resultingLabel);
                        ...
                        && (modifier.charAt(0) == '(')
                        && modifier.endsWith(")")) {
                    // Alternate text modifier in parentheses. Should be inserted if the label is
                    // empty
     ...
     */
    public static String joinAuthorsOnLastName(
            AuthorList authorList, int maxAuthors, String delimiter, final String suffix) {
    ...
    static String authEtal(AuthorList authorList, String delim, String append) {
        ...
        if ((authorList.getNumberOfAuthors() <= 2)
            ...
            // exception: If the second author is "and others", then do the appendix handling (in
            // the other branch)
    ...
    static String authIniN(AuthorList authorList, int n) {
        ...
        final int numberOfAuthors = authorList.getNumberOfAuthors();
        final boolean lastAuthorIsOthers =
                authorList.getAuthor(numberOfAuthors - 1).equals(Author.OTHERS);
        if (n >= numberOfAuthors && lastAuthorIsOthers) {
            ...
            AuthorList allButOthers =
                    AuthorList.of(authorList.getAuthors().stream().limit(limit).toList());
        ...
        for (int i = 0; i < arg.length(); i++) {
            ...
            if ((currentChar == ':') && !escaped && (inParenthesis == 0)) {
                ...
                if (escaped) {
                    ...
                    // " -> regex("...", "...") - escaping should be passed through to the regex
                    // parser
                    // : -> :abc:def
                    current.append('\\');
                    current.append('\\');
    ...
    static String generateInstitutionKey(String content) {
        ...
        String result =
                unicodeFormattedName.orElse(Normalizer.normalize(content, Normalizer.Form.NFC));

        // Special characters can't be allowed past this point because the citation key generator
        // might replace them with multiple mixed-case characters
    ...
    private static List<String> getValidInstitutionNameParts(String name) {
        ...
        for (String part : name.replaceAll("\\{[A-Z]+}", "").split("[ \\-_]")) {
            if ((!part.isEmpty() // remove empty
                            && !ignore.contains(
                                    part.toLowerCase(Locale.ENGLISH)) // remove ignored words
