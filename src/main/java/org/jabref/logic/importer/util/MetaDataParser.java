...
public class MetaDataParser {
    ...
    public static Optional<BibEntryType> parseCustomEntryType(String comment) {
        ...
        String optFields =
                fieldsDescription.substring(indexEndOfRequiredFields + 6, indexEndOfOptionalFields);
        BibEntryTypeBuilder entryTypeBuilder =
    ...
    public MetaData parse(MetaData metaData, Map<String, String> data, Character keywordSeparator)
        ...
        // process groups (GROUPSTREE and GROUPSTREE_LEGACY) at the very end (otherwise it can
        // happen that not all dependent data are set)
        ...
        for (Map.Entry<String, String> entry : entryList) {
            ...
            if (entry.getKey().startsWith(MetaData.PREFIX_KEYPATTERN)) {
                EntryType entryType =
                        EntryTypeFactory.parse(
                                entry.getKey().substring(MetaData.PREFIX_KEYPATTERN.length()));
                nonDefaultCiteKeyPatterns.put(
                        entryType, new CitationKeyPattern(getSingleItem(values)));
            } else if (entry.getKey().startsWith(MetaData.SELECTOR_META_PREFIX)) {
                // edge case, it might be one special field e.g. article from biblatex-apa, but we
                // can't distinguish this from any other field and rather prefer to handle it as
                // UnknownField
                metaData.addContentSelector(
                        ContentSelectors.parse(
                                FieldFactory.parseField(
                                        entry.getKey()
                                                .substring(MetaData.SELECTOR_META_PREFIX.length())),
    ...
    private static Optional<String> getNextUnit(Reader reader) throws IOException {
        ...
        while ((c = reader.read()) != -1) {
            if (escape) {
                ...
                if (character != MetaData.SEPARATOR_CHARACTER
    ...
    public static FieldFormatterCleanups fieldFormatterCleanupsParse(
            List<String> formatterMetaList) {
        if ((formatterMetaList != null) && (formatterMetaList.size() >= 2)) {
            boolean enablementStatus =
                    FieldFormatterCleanups.ENABLED.equals(formatterMetaList.getFirst());
            String formatterString = formatterMetaList.get(1);

            return new FieldFormatterCleanups(
                    enablementStatus, FieldFormatterCleanups.parse(formatterString));
        } else {
