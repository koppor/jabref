...
public class ScienceDirect implements FulltextFetcher, CustomizableKeyFetcher {
    ...
    public Optional<URL> findFullText(BibEntry entry) throws IOException {
        ...
        Optional<JSONObject> pdfDownloadOptional =
                html.getElementsByAttributeValue("type", "application/json").stream()
                        .flatMap(element -> element.getElementsByTag("script").stream())
                        // The first DOM child of the script element is the script itself
                        // (represented as HTML text)
        ...
        if (pdfDownload.has("linkToPdf")) {
            ...
            String queryParameters =
                    queryParamsObject.keySet().stream()
        ...
        try {
            ...
            HttpResponse<JsonNode> jsonResponse =
                    Unirest.get(request)
                            .header(
                                    "X-ELS-APIKey",
                                    importerPreferences.getApiKey(getName()).orElse(""))
                            .queryString("httpAccept", "application/json")
                            .asJson();

            JSONObject json = jsonResponse.getBody().getObject();
            JSONArray links =
                    json.getJSONObject("full-text-retrieval-response")
