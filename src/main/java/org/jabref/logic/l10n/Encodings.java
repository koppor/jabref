...
public class Encodings {
    ...
    private static List<Charset> encodingsList =
            Charset.availableCharsets().values().stream().distinct().collect(Collectors.toList());

    private Encodings() {}

    static {
        List<String> encodingsStringList =
                encodingsList.stream().map(Charset::displayName).distinct().collect(Collectors.toList());
