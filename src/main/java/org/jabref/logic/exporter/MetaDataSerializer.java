...
public class MetaDataSerializer {
     ...
     */
    public static Map<String, String> getSerializedStringMap(
            MetaData metaData, GlobalCitationKeyPatterns globalCiteKeyPatterns) {

        // metadata-key, list of contents
        //  - contents to be separated by OS.NEWLINE
        //  - each meta data item is written as separate @Comment entry - see
        // org.jabref.logic.exporter.BibtexDatabaseWriter.writeMetaDataItem
        Map<String, List<String>> stringyMetaData = new HashMap<>();

        // First write all meta data except groups
        metaData.getSaveOrder()
                .ifPresent(
                        saveOrderConfig ->
                                stringyMetaData.put(
                ...
                .ifPresent(
                        saveActions ->
                                stringyMetaData.put(
                                        ...
                                        getAsStringList(saveActions, OS.NEWLINE)));
                ...
                .ifPresent(
                        mode ->
                                stringyMetaData.put(
                                        ...
                                        Collections.singletonList(mode.getAsString())));
        ...
        metaData.getUserFileDirectories()
                .forEach(
                        (user, path) ->
                                stringyMetaData.put(
                                        MetaData.FILE_DIRECTORY + '-' + user,
                                        Collections.singletonList(path.trim())));
        metaData.getLatexFileDirectories()
                .forEach(
                        (user, path) ->
                                stringyMetaData.put(
                                        MetaData.FILE_DIRECTORY_LATEX + '-' + user,
                                        Collections.singletonList(path.toString().trim())));
        metaData.getVersionDBStructure()
                .ifPresent(
                        versionDBStructure ->
                                stringyMetaData.put(
                                        MetaData.VERSION_DB_STRUCT,
                                        Collections.singletonList(versionDBStructure.trim())));

        for (ContentSelector selector : metaData.getContentSelectorsSorted()) {
            stringyMetaData.put(
                    MetaData.SELECTOR_META_PREFIX + selector.getField().getName(),
                    selector.getValues());
        }
        ...
        metaData.getGroupSearchSyntaxVersion()
                .ifPresent(
                        version ->
                                serializedMetaData.put(
                                        MetaData.GROUPS_SEARCH_SYNTAX_VERSION, version.toString()));
    ...
    private static Map<String, String> serializeMetaData(
        ...
        for (Map.Entry<String, List<String>> metaItem : stringyMetaData.entrySet()) {
            ...
            // The last "MetaData.SEPARATOR_STRING" adds compatibility to JabRef v5.9 and earlier
            StringJoiner joiner =
                    new StringJoiner(MetaData.SEPARATOR_STRING, "", MetaData.SEPARATOR_STRING);
            ...
            for (String dataItem : itemList) {
                ...
                string +=
                        StringUtil.quote(
                                dataItem, MetaData.SEPARATOR_STRING, MetaData.ESCAPE_CHARACTER);
    ...
    private static Map<String, List<String>> serializeCiteKeyPatterns(
            MetaData metaData, GlobalCitationKeyPatterns globalCitationKeyPatterns) {
        Map<String, List<String>> stringyPattern = new HashMap<>();
        AbstractCitationKeyPatterns citationKeyPattern =
                metaData.getCiteKeyPatterns(globalCitationKeyPatterns);
        ...
        if ((citationKeyPattern.getDefaultValue() != null)
                && !citationKeyPattern
                        .getDefaultValue()
    ...
    private static String serializeGroups(GroupTreeNode root) {
        ...
        for (String groupNode : new GroupSerializer().serializeTree(root)) {
            stringBuilder.append(
                    StringUtil.quote(
                            groupNode, MetaData.SEPARATOR_STRING, MetaData.ESCAPE_CHARACTER));
    ...
    public static String serializeCustomEntryTypes(BibEntryType entryType) {
        return MetaData.ENTRYTYPE_FLAG
                ...
                + FieldFactory.serializeFieldsList(
                        entryType.getOptionalFields().stream().map(BibField::field).toList())
                + "]";
    }

    public static List<String> getAsStringList(
            FieldFormatterCleanups fieldFormatterCleanups, String delimiter) {
