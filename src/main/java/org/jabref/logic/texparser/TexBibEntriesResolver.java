...
public class TexBibEntriesResolver {
    ...
    public LatexBibEntriesResolverResult resolve(LatexParserResults latexParserResults) {
        ...
        List<BibDatabase> bibDatabases =
                latexParserResults.getBibFiles().stream()
                        .map(
                                path -> {
                                    try {
                                        return OpenDatabase.loadDatabase(
                                                        path, importFormatPreferences, fileMonitor)
                                                .getDatabase();
                                    } catch (IOException e) {
        ...
        List<BibEntry> newEntries =
                findNewEntries(bibDatabases, latexParserResults.getCitations().keySet());
                        ...
                        database ->
                                citations.stream()
                                        .flatMap(
                                                citation ->
                                                        database
                                                                .getEntriesByCitationKey(citation)
                                                                .stream())
                                        // Check if there is already an entry with the same key in
                                        // the given database.
                                        .filter(
                                                entry ->
                                                        !entry.equals(
                                                                masterDatabase
                                                                        .getEntryByCitationKey(
                                                                                entry.getCitationKey()
                                                                                        .orElse(""))
                                                                        .orElse(new BibEntry())))
                                        // Add cross-referencing data to the entry (fill empty
                                        // fields).
