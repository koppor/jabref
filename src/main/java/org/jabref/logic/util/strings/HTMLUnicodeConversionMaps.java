...
public class HTMLUnicodeConversionMaps {
    ...
    // The symbols can be seen at http://www.fileformat.info/info/unicode/char/a4/index.htm. Replace
    // "a4" with the U+ number
    // http://detexify.kirelabs.org/classify.html and
    // http://www.ctan.org/tex-archive/info/symbols/comprehensive/ might help to find the right
    // LaTeX command
    ...
    */
    ...
    private static final String[][] CONVERSION_LIST =
            new String[][] {
                ...
                //                                    = carriage return, U+21B5 NEW - require
                // mathabx
                ...
                {
                    "8249", "lsaquo", "{\\guilsinglleft}"
                }, // single left-pointing angle quotation mark,
                //                                   U+2039 ISO proposed
                /* lsaquo is proposed but not yet ISO standardized */
                {
                    "8250", "rsaquo", "{\\guilsinglright}"
                ...
                {
                    "37", "#37", "\\%"
                }, // Percent (&percnt; is not displayed correctly in the entry preview)
                {"39", "apos", "'"}, // Apostrophe
                ...
                {"8334", "", "$_{)}$"}, // sub-script )
                {
                    "8450", "complexes", "$\\mathbb{C}$"
                }, // double struck capital C -- requires e.g. amsfonts
                {"8451", "", "{\\textcelsius}"}, // Degree Celsius
                {"8459", "Hscr", "{{$\\mathcal{H}$}}"}, // script capital H -- possibly use \mathscr
                {
                    "8460", "Hfr", "{{$\\mathbb{H}$}}"
                }, // black letter capital H -- requires e.g. amsfonts
                {"8466", "Lscr", "{{$\\mathcal{L}$}}"}, // script capital L -- possibly use \mathscr
                {"8467", "ell", "{$\\ell$}"}, // script small l
                {
                    "8469", "naturals", "{{$\\mathbb{N}$}}"
                }, // double struck capital N -- requires e.g. amsfonts
                {
                    "8474", "Qopf", "{{$\\mathbb{Q}$}}"
                }, // double struck capital Q -- requires e.g. amsfonts
                {
                    "8477", "reals", "{{$\\mathbb{R}$}}"
                }, // double struck capital R -- requires e.g. amsfonts
                {"8486", "", "${{\\Omega}}$"}, // Omega
                ...
                {
                    "9653", "utri", "$\\triangle$"
                }, // White up-pointing small triangle -- \vartriangle probably
                ...
                {
                    "120598", "", "$\\epsilon$"
                }, // mathematical italic epsilon U+1D716 -- requires amsmath
                {"120599", "", "{{\\˜{n}}}"}, // n with tide
            };

    // List of combining accents
    private static final String[][] ACCENT_LIST =
            new String[][] {
                {"768", "`"}, // Grave
                {"769", "'"}, // Acute
                {"770", "^"}, // Circumflex
    ...
    static {
        for (String[] aConversionList : CONVERSION_LIST) {
            if (!aConversionList[2].isEmpty()) {
                ...
                if (!aConversionList[1].isEmpty()) {
                    HTML_LATEX_CONVERSION_MAP.put(
                            "&" + aConversionList[1] + ";", aConversionList[2]);
                    if (!strippedLaTeX.isEmpty()) {
                        LATEX_HTML_CONVERSION_MAP.put(
                                strippedLaTeX, "&" + aConversionList[1] + ";");
                    }
                ...
                if (!aConversionList[0].isEmpty()) {
                    NUMERICAL_LATEX_CONVERSION_MAP.put(
                            Integer.decode(aConversionList[0]), aConversionList[2]);
        ...
        // Manual corrections
        LATEX_HTML_CONVERSION_MAP.put(
                "AA", "&Aring;"); // Overwritten by &angst; which is less supported
