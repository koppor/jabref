...
public class FileUtil {
        ...
        if (nameWithoutExtension.length() > MAXIMUM_FILE_NAME_LENGTH) {
            ...
            LOGGER.info(
                    "Truncated the too long filename '%s' (%d characters) to '%s'."
                            .formatted(fileName, fileName.length(), shortName));
    ...
    public static boolean copyFile(
            Path pathToSourceFile, Path pathToDestinationFile, boolean replaceExisting) {
        ...
        try {
            ...
            Files.copy(
                    pathToSourceFile, pathToDestinationFile, StandardCopyOption.REPLACE_EXISTING);
            return true;
        } catch (IOException e) {
    ...
    public static Path relativize(
            Path path, BibDatabaseContext databaseContext, FilePreferences filePreferences) {
     ...
     */
    public static List<BibEntry> relativize(
                ...
                .map(
                        entry -> {
                            if (entry.hasField(StandardField.FILE)) {
                                List<LinkedFile> updatedLinkedFiles =
    ...
    public static String createDirNameFromPattern(
            BibDatabase database, BibEntry entry, String directoryNamePattern) {
        String targetName =
                BracketedPattern.expandBrackets(directoryNamePattern, ';', entry, database);
    ...
    public static Optional<Path> findSingleFileRecursively(String filename, Path rootDirectory) {
        try (Stream<Path> pathStream = Files.walk(rootDirectory)) {
            ...
            LOGGER.error(
                    "Error trying to locate the file {} inside the directory {}",
                    filename,
                    rootDirectory);
        }
        return Optional.empty();
    ...
    public static Optional<Path> find(String fileName, Path directory) {
        ...
        // Explicitly check for an empty string, as File.exists returns true on that empty path,
        // because it maps to the default jar location.
        // If we then call toAbsoluteDir, it would always return the jar-location folder. This is
        ...
        // get the furthest path element from root and check if our filename starts with the same
        // name
    ...
    public static Path getInitialDirectory(
            BibDatabaseContext databaseContext, Path workingDirectory) {
    ...
    public static boolean detectBadFileName(String fileName) {
        // fileName could be a path, we want to check the fileName only (and don't care about the
        // path)
        // Reason: Handling of "c:\temp.pdf" is difficult, because ":" is an illegal character in
        // the file name,
