...
public class ConvertToBiblatexCleanup implements CleanupJob {
    ...
    public List<FieldChange> cleanup(BibEntry entry) {
        ...
        for (Map.Entry<Field, Field> alias :
                EntryConverter.FIELD_ALIASES_BIBTEX_TO_BIBLATEX.entrySet()) {
            Field oldField = alias.getKey();
            Field newField = alias.getValue();
            entry.getField(oldField)
                    .ifPresent(
                            oldValue -> {
                                if (!oldValue.isEmpty() && (entry.getField(newField).isEmpty())) {
                                    // There is content in the old field and no value in the new, so
                                    // just copy
        ...
        if (StringUtil.isBlank(entry.getField(StandardField.DATE))) {
            entry.getFieldOrAlias(StandardField.DATE)
                    .ifPresent(
                            newDate -> {
            ...
            // If the year from date field is filled and equal to year it should be removed the year
            // field
            entry.getFieldOrAlias(StandardField.DATE)
                    .ifPresent(
                            date -> {
                                Optional<Date> newDate = Date.parse(date);
                                Optional<Date> checkDate =
                                        Date.parse(
