...
public class Pseudonymization {
    ...
    public Result pseudonymizeLibrary(BibDatabaseContext bibDatabaseContext) {
        ...
        fieldToValueToIdMap.forEach(
                (field, stringToIntMap) -> {
                    stringToIntMap.forEach(
                            (value, id) -> {
                                valueMapping.put(
                                        field.getName().toLowerCase(Locale.ROOT) + "-" + id, value);
                            });
                });
     ...
     */
    private static List<BibEntry> pseudonymizeEntries(
        ...
        for (BibEntry entry : entries) {
            ...
            for (Field field : entry.getFields()) {
                ...
                // TODO: Use {@link org.jabref.model.entry.field.FieldProperty} to distinguish
                // cases.
                //       See {@link org.jabref.model.entry.field.StandardField} for usages.
                String fieldContent = entry.getField(field).get();
                Integer id =
                        valueToIdMap.computeIfAbsent(fieldContent, k -> valueToIdMap.size() + 1);
