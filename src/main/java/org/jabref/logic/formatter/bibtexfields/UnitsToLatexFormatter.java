...
public class UnitsToLatexFormatter extends Formatter {
    ...
    private static final List<String> UNIT_LIST =
            Arrays.asList(
                    "A", // Ampere
                    "Ah", // Ampere hours
                    "B", // Byte
    ...
    private static final List<String> UNIT_PREFIX_LIST =
            Arrays.asList(
                    "y", // yocto
                    "z", // zepto
                    "a", // atto
    ...
    public UnitsToLatexFormatter() {
        ...
        Collections.sort(
                prefixUnitCombinations,
                new StringLengthComparator()); // Sort based on string length
    }
    ...
    public String format(String text) {
        ...
        // Replace the hyphen in 12-bit etc with a non-breaking hyphen, will also avoid bad casing
        // of 12-Bit
        String result = text.replaceAll("([0-9,\\.]+)-([Bb][Ii][Tt])", "$1\\\\mbox\\{-\\}$2");

        // Replace the space in 12 bit etc with a non-breaking space, will also avoid bad casing of
        // 12 Bit
        result = result.replaceAll("([0-9,\\.]+) ([Bb][Ii][Tt])", "$1~$2");

        // For each word in the list
        for (String listOfWord : prefixUnitCombinations) {
            // Add {} if the character before is a space, -, /, (, [, or } or if it is at the start
            // of the string but not if it is followed by a }
            result =
                    result.replaceAll(
                            "([0-9])(" + listOfWord + ")",
                            "$1\\{$2\\}"); // Only add brackets to keep case
            result =
                    result.replaceAll(
                            "([0-9])-(" + listOfWord + ")",
                            "$1\\\\mbox\\{-\\}\\{$2\\}"); // Replace hyphen with non-break hyphen
            result =
                    result.replaceAll(
                            "([0-9]) (" + listOfWord + ")",
                            "$1~\\{$2\\}"); // Replace space with a hard space
        }
