...
public class LatexCleanupFormatter extends Formatter {
    ...
    private static final Pattern REMOVE_REDUNDANT =
            Pattern.compile("(?<!\\\\[\\p{Alpha}]{0,100}\\{[^\\}]{0,100})\\}([-/ ]?)\\{");
    ...
    public String format(String oldString) {
        ...
        // Remove redundant $, {, and }, but not if the } is part of a command argument: \mbox{-}{GPS} should not be
        // adjusted
        ...
        // newValue = newValue.replaceAll("([0-9\\(\\.]+) \\$","\\$$1\\\\ "); // Move numbers followed by a space left
        // of $ inside the equation, e.g., 0.35 $\mu$m

        newValue = MOVE_NUMBERS_WITH_OPERATORS
                .matcher(newValue)
                .replaceAll("\\$$1"); // Move numbers, possibly with operators +, -, or /,  left of $ into the equation
        newValue = MOVE_NUMBERS_RIGHT_INTO_EQUATION
                .matcher(newValue)
        ...
        newValue = ESCAPE_PERCENT_SIGN_ONCE
                .matcher(newValue)
                .replaceAll("$1\\\\%"); // escape %, but do not escapee \% again,  used for comments in TeX
