...
public class LatexCleanupFormatter extends Formatter {
    ...
    private static final Pattern MOVE_NUMBERS_WITH_OPERATORS =
            Pattern.compile("([0-9\\(\\.]+[ ]?[-+/]?[ ]?)\\$");
    ...
    public String format(String oldString) {
        ...
        // \mbox{-}{GPS} should not be adjusted
        ...
        newValue =
                REPLACE_WITH_AT
                        .matcher(newValue)
                        .replaceAll("$1@@"); // Replace $, but not \$ with @@

        newValue =
                REPLACE_EVERY_OTHER_AT
                        .matcher(newValue)
                        .replaceAll("$1\\$$2@@"); // Replace every other @@ with $
        // newValue = newValue.replaceAll("([0-9\\(\\.]+) \\$","\\$$1\\\\ "); // Move numbers
        // followed by a space left of $ inside the equation, e.g., 0.35 $\mu$m

        newValue =
                MOVE_NUMBERS_WITH_OPERATORS
                        .matcher(newValue)
                        .replaceAll(
