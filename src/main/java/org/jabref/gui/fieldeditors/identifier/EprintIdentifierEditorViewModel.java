...
public class EprintIdentifierEditorViewModel
    ...
    // The following listener will be wrapped in a weak reference change listener, thus it will be
    // garbage collected
        ...
        EasyBind.subscribe(
                ...
                newIdentifier -> {
                    newIdentifier.ifPresent(
                            id -> {
                                // TODO: We already have a common superclass between ArXivIdentifier
                                // and ARK. This could be refactored further.
    ...
    public void bindToEntry(BibEntry entry) {
        ...
        // Unlike other identifiers (they only depend on their own field value), eprint  depends on
        // eprinttype thus
        // its identity changes whenever the eprinttype field changes .e.g. If eprinttype equals
        // 'arxiv' then the eprint identity
        // will be of type ArXivIdentifier and if it equals 'ark' then it switches to type ARK.
        entry.getFieldsObservable()
                .addListener(new WeakMapChangeListener<>(eprintTypeFieldListener));
    }
}
