...
public class FieldRowView {
    ...
    public FieldRowView(
        ...
        viewModel =
                new FieldRowViewModel(
                        field, leftEntry, rightEntry, mergedEntry, fieldMergerFactory);
        ...
        // As a workaround we need to have a reference to the parent grid pane to be able to
        // show/hide the row.
        ...
        EasyBind.subscribe(
                viewModel.isFieldsMergedProperty(),
                isFieldsMerged -> {
                    if (isFieldsMerged) {
        ...
        EasyBind.listen(
                viewModel.hasEqualLeftAndRightBinding(),
                (obs, old, isEqual) -> {
                    if (isEqual) {
                        LOGGER.debug(
                                "Left and right values are equal, LEFT==RIGHT=={}",
        ...
        if (shouldShowDiffs.get()) {
            if (diffConfig.diffView() == ThreeWayMergeToolbar.DiffView.UNIFIED) {
                new UnifiedDiffHighlighter(
                                leftLabel, rightLabel, diffConfig.diffHighlightingMethod())
                        .highlight();
            } else {
    ...
    public String toString() {
        return "FieldRowView [shouldShowDiffs="
                + shouldShowDiffs.get()
                + ", fieldNameCell="
                + fieldNameCell
                + ", leftValueCell="
