...
public class MultiMergeEntriesView extends BaseDialog<BibEntry> {
    ...
    public MultiMergeEntriesView(GuiPreferences preferences, TaskExecutor taskExecutor) {
        ...
        viewModel
                .entriesProperty()
                .addListener(
                        (ListChangeListener<MultiMergeEntriesViewModel.EntrySource>)
                                c -> {
                                    while (c.next()) {
                                        if (c.wasAdded()) {
                                            for (MultiMergeEntriesViewModel.EntrySource
                                                    entrySourceColumn : c.getAddedSubList()) {
        ...
        viewModel
                ...
                .getFieldsObservable()
                .addListener(
                        (MapChangeListener<Field, String>)
                                change -> {
                                    if (change.wasAdded()
    ...
    private void fillDiffModes() {
        ...
        EasyBind.subscribe(
                ...
                mode -> preferences.getMergeDialogPreferences().setMergeDiffMode(mode));
    }
    ...
    private ToggleButton generateEntryHeader(
            MultiMergeEntriesViewModel.EntrySource column, int columnIndex) {
    ...
    private void writeBibEntryToColumn(
            MultiMergeEntriesViewModel.EntrySource entrySourceColumn, int columnIndex) {
        ...
        public Cell(String content, Field field, int columnIndex) {
            ...
            UiTaskExecutor.runInJavaFXThread(
                    () -> {
                        ...
                        prefWidthProperty()
                                .bind(
                                        ((Region) supplierHeader.getChildren().get(columnIndex))
                                                .widthProperty());
                        setMinWidth(Control.USE_PREF_SIZE);
                        setMaxWidth(Control.USE_PREF_SIZE);
                        prefHeightProperty()
                                .bind(
                                        ((Region) fieldEditor.getChildren().get(row.rowIndex))
                                                .heightProperty());
    ...
    private class FieldRow {
        ...
        public FieldRow(Field field, int rowIndex) {
            ...
            fieldEditorCell.addEventFilter(
                    KeyEvent.KEY_PRESSED, event -> toggleGroup.selectToggle(null));
