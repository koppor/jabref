...
public class ExtractReferencesAction extends SimpleCommand {
    ...
    public ExtractReferencesAction(
            DialogService dialogService, StateManager stateManager, CliPreferences preferences) {
        ...
        stateManager
                ...
                .ifPresent(
                        databaseContext -> {
                            ...
                            if (online) {
                                Optional<Callable<ParserResult>> parserResultCallableOnline =
                                        getParserResultCallableOnline(
                                                databaseContext, selectedEntries);
                                ...
                                parserResultCallable =
                                        getParserResultCallableOffline(
                                                databaseContext, selectedEntries);
                            }
                            BackgroundTask<ParserResult> task =
                                    BackgroundTask.wrap(parserResultCallable)
                                            .withInitialMessage(
                                                    Localization.lang("Processing PDF(s)"));

                            task.onFailure(dialogService::showErrorDialogAndWait);

                            ImportEntriesDialog dialog =
                                    new ImportEntriesDialog(
                                            stateManager.getActiveDatabase().get(), task);
                            String title;
                            if (online) {
    ...
    private @NonNull Callable<ParserResult> getParserResultCallableOffline(
            BibDatabaseContext databaseContext, List<BibEntry> selectedEntries) {
        return () -> {
            ...
            // We need to have ParserResult handled at the importer, because it imports the meta
            // data (library type, encoding, ...)
    ...
    private void extractReferences(
            Iterator<Path> fileListIterator, ParserResult result, BibEntry currentEntry) {
    ...
    private static String getCites(List<BibEntry> entries, BibEntry currentEntry) {
        ...
        for (BibEntry importedEntry : entries) {
            ...
            if (citationKey.isPresent()) {
                ...
                //   the number of the current entry (extracted from the reference; fallback:
                // current number of the entry (count variable))
                ...
                // We use the number of the comment field (because there is no other way to get the
                // number reliable)
                Pattern pattern = Pattern.compile("^\\[(\\d+)\\]");
                Matcher matcher =
                        pattern.matcher(importedEntry.getField(StandardField.COMMENT).orElse(""));
                if (matcher.hasMatch()) {
    ...
    private Optional<Callable<ParserResult>> getParserResultCallableOnline(
        ...
        return Optional.of(
                () ->
                        new ParserResult(
                                new GrobidService(this.preferences.getGrobidPreferences())
                                        .processReferences(
                                                fileList,
