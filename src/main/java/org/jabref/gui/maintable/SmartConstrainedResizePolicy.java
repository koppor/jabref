...
public class SmartConstrainedResizePolicy implements Callback<TableView.ResizeFeatures, Boolean> {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(SmartConstrainedResizePolicy.class);
    ...
    private Boolean initColumnSize(TableView<?> table) {
        ...
        double totalWidth =
                visibleLeafColumns.stream().mapToDouble(TableColumnBase::getWidth).sum();

        if (Math.abs(totalWidth - tableWidth) > 1) {
            double totalPrefWidth =
                    visibleLeafColumns.stream().mapToDouble(TableColumnBase::getPrefWidth).sum();
            ...
            if (totalPrefWidth > 0) {
                for (TableColumnBase col : visibleLeafColumns) {
                    ...
                    // Just to make sure that we are staying under the total table width (due to
                    // rounding errors)
    ...
    private void resize(TableColumnBase column, double delta) {
        ...
        try {
            ...
            Method constrainedResize =
                    clazz.getDeclaredMethod("resize", TableColumnBase.class, double.class);
    ...
    private Boolean constrainedResize(
        ...
        try {
            ...
            constrainedResize.setAccessible(true);
            Object returnValue =
                    constrainedResize.invoke(
                            null, prop, isFirstRun, contentWidth, visibleLeafColumns);
