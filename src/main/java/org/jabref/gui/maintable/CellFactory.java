...
public class CellFactory {
    ...
    public CellFactory(GuiPreferences preferences, UndoManager undoManager) {
        ...
        for (ExternalFileType fileType :
                preferences.getExternalApplicationsPreferences().getExternalFileTypes()) {
            icon = fileType.getIcon();
            // icon.setToolTipText(Localization.lang("Open %0 file", fileType.getName()));
            TABLE_ICONS.put(fileType.getField(), icon);
        }

        SpecialFieldViewModel relevanceViewModel =
                new SpecialFieldViewModel(SpecialField.RELEVANCE, preferences, undoManager);
        ...
        SpecialFieldViewModel qualityViewModel =
                new SpecialFieldViewModel(SpecialField.QUALITY, preferences, undoManager);
        ...
        SpecialFieldViewModel rankViewModel =
        ...
        SpecialFieldViewModel priorityViewModel =
                new SpecialFieldViewModel(SpecialField.PRIORITY, preferences, undoManager);
        ...
        SpecialFieldViewModel readViewModel =
                new SpecialFieldViewModel(SpecialField.READ_STATUS, preferences, undoManager);
        ...
        SpecialFieldViewModel printedViewModel =
                new SpecialFieldViewModel(SpecialField.PRINTED, preferences, undoManager);
    ...
    public Node getTableIcon(Field field) {
        ...
        if (icon == null) {
            ...
            // node should be generated for each call, as nodes can be added to the scene graph only
            // once
