...
public class MainTableHeaderContextMenu extends ContextMenu {
    ...
    public void show(boolean show) {
        // TODO: 20/10/2022 unknown bug where issue does not show unless parameter is passed through
        // this method.
        mainTable.setOnContextMenuRequested(
                event -> {
    ...
    private void constructItems() {
        ...
        for (TableColumn<BibEntryTableViewModel, ?> column : mainTable.getColumns()) {
            if (((MainTableColumn<?>) column).getModel().getType()
    ...
    private RightClickMenuItem createMenuItem(
            TableColumn<BibEntryTableViewModel, ?> column, boolean isDisplaying) {
    ...
    private List<TableColumn<BibEntryTableViewModel, ?>> commonColumns() {
        ...
        commonColumns.add(
                new MainTableColumnModel(
                        MainTableColumnModel.Type.NORMALFIELD, entryTypeQualifier));
        commonColumns.add(
                new MainTableColumnModel(
                        MainTableColumnModel.Type.NORMALFIELD, authorEditQualifier));
        commonColumns.add(
        ...
        commonColumns.add(
                new MainTableColumnModel(
                        MainTableColumnModel.Type.NORMALFIELD, journalBookQualifier));
        commonColumns.add(
                new MainTableColumnModel(MainTableColumnModel.Type.SPECIALFIELD, rankQualifier));
        commonColumns.add(
                new MainTableColumnModel(
                        MainTableColumnModel.Type.SPECIALFIELD, readStatusQualifier));
        commonColumns.add(
                new MainTableColumnModel(
                        MainTableColumnModel.Type.SPECIALFIELD, priorityQualifier));
