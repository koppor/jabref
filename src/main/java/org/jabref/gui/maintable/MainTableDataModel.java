...
public class MainTableDataModel {
                ...
                EasyBind.mapBacked(
                        ...
                        entry ->
                                new BibEntryTableViewModel(
                                        entry, bibDatabaseContext, fieldValueFormatter),
                        false);
    ...
    private void setSearchMatches(SearchResults results) {
        ...
        entriesViewModel.forEach(
                entry -> {
                    entry.searchScoreProperty()
                            .set(results.getSearchScoreForEntry(entry.getEntry()));
                    entry.hasFullTextResultsProperty()
                            .set(results.hasFulltextResults(entry.getEntry()));
                    updateEntrySearchMatch(
                            entry, entry.searchScoreProperty().get() > 0, isFloatingMode);
                });
    }
    ...
    private static void updateEntrySearchMatch(
            BibEntryTableViewModel entry, boolean isMatched, boolean isFloatingMode) {
    ...
    private static Optional<MatcherSet> createGroupMatcher(
            List<GroupTreeNode> selectedGroups, GroupsPreferences groupsPreferences) {
        if ((selectedGroups == null) || selectedGroups.isEmpty()) {
            // No selected group, show all entries
            return Optional.empty();
        }

        final MatcherSet searchRules =
                MatcherSets.build(
                                                        ...
                                                        if (searchQueryProperty.get().isPresent()) {
                                                            ...
                                                            String newSearchExpression =
                                                                    "+"
                                                                            ...
                                                                            + searchQuery
                                                                                    .getSearchExpression();
                                                            SearchQuery entryQuery =
                                                                    new SearchQuery(
                                                                            newSearchExpression,
                                                                            searchQuery
                                                                                    .getSearchFlags());
                                                            SearchResults results =
                                                                    luceneManager.search(
                                                                            entryQuery);
