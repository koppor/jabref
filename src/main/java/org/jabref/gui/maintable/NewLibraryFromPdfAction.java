...
public abstract class NewLibraryFromPdfAction extends SimpleCommand {
    ...
    public void execute() {
        ...
        // Sensible default for the directory to start browsing is the directory of the currently
        // opened library. The pdf storage dir seems not to be feasible, because extracting
        // references from a PDF itself can be done by the context menu of the respective entry.
        stateManager
                .getActiveDatabase()
                        ...
                        path -> {
                            ...
                            BackgroundTask.wrap(parserResultCallable)
                                    ...
                                    .onSuccess(
                                            result -> {
                                                LOGGER.trace(
                                                        "Finished processing PDF(s): {}", result);
                                                libraryTabContainer.addTab(
                                                        result.getDatabaseContext(), true);
                                            })
