...
public class ChatPromptComponent extends HBox {
    ...
    private final ListProperty<String> history =
    ...
    private final IntegerProperty currentUserMessageScroll =
            new SimpleIntegerProperty(NEW_NON_EXISTENT_MESSAGE);
    ...
    private void initialize() {
        userPromptTextArea.setOnKeyPressed(
                keyEvent -> {
                    if (keyEvent.getCode() == KeyCode.DOWN) {
                        ...
                        if (keyEvent.getCode() != KeyCode.RIGHT
                            ...
                            // It is okay to go back and forth in the prompt while showing a history
                            // message.
        ...
        currentUserMessageScroll.addListener(
                (observable, oldValue, newValue) -> {
                    // When currentUserMessageScroll is reset, then its value is
                    // 1) either to NEW_NON_EXISTENT_MESSAGE,
                    // 2) or to a new history entry.
                    if (newValue.intValue() != NEW_NON_EXISTENT_MESSAGE
                            && showingHistoryMessage.get()) {
                        if (userPromptTextArea.getCaretPosition() == 0
                            ...
                            // But the exception to this situation is when the caret position is at
                        ...
                        // When currentUserMessageScroll is set to NEW_NON_EXISTENT_MESSAGE, then we
                        // should:
                        // 1) either clear the prompt, if user scrolls down the most recent history
                        // entry.
    ...
    public void requestPromptFocus() {
        // TODO: Check what would happen when programmer calls requestPromptFocus() while the
