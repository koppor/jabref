...
public class LinkedFileEditDialogViewModel extends AbstractViewModel {
    ...
    public LinkedFileEditDialogViewModel(
    ...
    public void setValues(LinkedFile linkedFile) {
        ...
        Optional<ExternalFileType> fileType =
                ExternalFileTypes.getExternalFileTypeByLinkedFile(
                        linkedFile, false, externalApplicationsPreferences);
        ...
        if (LinkedFile.isOnlineLink(link.getValue())) {
            try {
                return new LinkedFile(
                        ...
                        URI.create(link.getValue()).toURL(),
                        fileType,
                        sourceUrl.getValue());
            } catch (MalformedURLException e) {
                return new LinkedFile(
                        description.getValue(), link.getValue(), fileType, sourceUrl.getValue());
            }
        }
        return new LinkedFile(
                description.getValue(), Path.of(link.getValue()), fileType, sourceUrl.getValue());
    }
