...
public class CustomImporterTabViewModel implements PreferenceTabViewModel {
    ...
    public void setValues() {
        Set<CustomImporter> importersLogic =
                preferences.getImporterPreferences().getCustomImporters();
        importers.clear();
    ...
    private static String pathToClass(String basePath, Path path) {
        String className =
                FileUtil.relativize(path, Collections.singletonList(Path.of(basePath))).toString();
        if (className != null) {
    ...
    public void addImporter() {
        FileDialogConfiguration fileDialogConfiguration =
                new FileDialogConfiguration.Builder()
                        .addExtensionFilter(
                                StandardFileType.CLASS, StandardFileType.JAR, StandardFileType.ZIP)
        ...
        if (selectedFile.isPresent() && (selectedFile.get().getParent() != null)) {
            boolean isArchive =
                    FileUtil.getFileExtension(selectedFile.get())
                            .filter(
                                    extension ->
                ...
                try {
                    ...
                    if (selectedFileInArchive.isPresent()) {
                        ...
                        CustomImporter importer =
                                new CustomImporter(
                                        selectedFile.get().toAbsolutePath().toString(), className);
                        importers.add(new ImporterViewModel(importer));
                    }
                    ...
                    dialogService.showErrorDialogAndWait(
                            Localization.lang("Could not instantiate %0 %1", "importer"), exc);
                }
            } else {
                try {
                    ...
                    dialogService.showErrorDialogAndWait(
                            Localization.lang(
                                    "Could not instantiate %0", selectedFile.get().toString()),
                            exc);
