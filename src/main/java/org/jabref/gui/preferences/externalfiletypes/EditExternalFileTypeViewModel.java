...
public class EditExternalFileTypeViewModel {
    ...
    private void setupValidation() {
        ...
        Validator extensionisNotBlankValidator =
                new FunctionBasedValidator<>(
                        ...
                        ValidationMessage.error(
                                Localization.lang("Please enter a name for the extension.")));
        Validator sameExtensionValidator =
                new FunctionBasedValidator<>(
                        extensionProperty,
                        extension -> {
        ...
        Validator sameNameValidator =
                new FunctionBasedValidator<>(
                        nameProperty,
                        name -> {
                            for (ExternalFileTypeItemViewModel fileTypeItem : fileTypes) {
                                if (name.equalsIgnoreCase(fileTypeItem.nameProperty().get())
        ...
        Validator nameIsNotBlankValidator =
                new FunctionBasedValidator<>(
                        nameProperty,
                        StringUtil::isNotBlank,
        ...
        Validator mimeTypeIsNotBlankValidator =
                new FunctionBasedValidator<>(
                        mimeTypeProperty,
                        StringUtil::isNotBlank,
                        ValidationMessage.error(
                                Localization.lang("Please enter a name for the MIME type.")));

        Validator sameMimeTypeValidator =
                new FunctionBasedValidator<>(
                        mimeTypeProperty,
                        mimeType -> {
                            for (ExternalFileTypeItemViewModel fileTypeItem : fileTypes) {
                                if (mimeType.equalsIgnoreCase(fileTypeItem.mimetypeProperty().get())
                        ...
                        ValidationMessage.error(
                                Localization.lang(
                                        "There already exists an external file type with the same MIME type")));
        mimeTypeValidator.addValidators(sameMimeTypeValidator, mimeTypeIsNotBlankValidator);

        validator.addValidators(
                extensionValidator,
                sameExtensionValidator,
                nameValidator,
                sameNameValidator,
                mimeTypeValidator,
                sameMimeTypeValidator);
    }
    ...
    public void storeSettings() {
        ...
        // store application as empty if the "Default" option is selected, or if the application
        // name is empty:
