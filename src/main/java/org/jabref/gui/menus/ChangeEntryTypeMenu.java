...
public class ChangeEntryTypeMenu {
    ...
    private ObservableList<MenuItem> getMenuItems(
            List<BibEntry> entries, BibDatabaseContext bibDatabaseContext, UndoManager undoManager) {
    ...
    private Optional<Menu> createSubMenu(
            String text, List<BibEntryType> entryTypes, List<BibEntry> entries, UndoManager undoManager) {
        Menu subMenu = null;
    ...
    private List<MenuItem> fromEntryTypes(
        ...
        return types.stream()
                ...
                .map(type -> factory.createMenuItem(
                        type::getDisplayName, new ChangeEntryTypeAction(type, entries, undoManager)))
                .toList();
    }
}
