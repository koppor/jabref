...
public class UnlinkedFilesDialogView extends BaseDialog<Void> {
    ...
    private void initialize() {
        viewModel = new UnlinkedFilesDialogViewModel(
                dialogService, undoManager, fileUpdateMonitor, preferencesService, stateManager, taskExecutor);

        this.bibDatabaseContext =
                stateManager.getActiveDatabase().orElseThrow(() -> new NullPointerException("No active library"));
    ...
    private void initDirectorySelection() {
        ...
        Platform.runLater(() ->
                validationVisualizer.initVisualization(viewModel.directoryPathValidationStatus(), directoryPathField));
        ...
        directoryPathField.setText(bibDatabaseContext
                .getFirstExistingFileDir(preferencesService.getFilePreferences())
                .map(Path::toString)
                .orElse(""));
    }
    ...
    private void initUnlinkedFilesList() {
        ...
        unlinkedFilesList
                .maxHeightProperty()
                .bind(((Control) filePane.contentProperty().get()).heightProperty());
        unlinkedFilesList.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
        unlinkedFilesList.rootProperty().bind(EasyBind.map(viewModel.treeRootProperty(), fileNode -> fileNode.map(
                        fileNodeViewModel -> new RecursiveTreeItem<>(fileNodeViewModel, FileNodeViewModel::getChildren))
                .orElse(null)));
        ...
        EasyBind.subscribe(unlinkedFilesList.rootProperty(), root -> {
            if (root != null) {
                ...
                EasyBind.bindContent(
                        viewModel.checkedFileListProperty(),
    ...
    private ContextMenu createSearchContextMenu() {
        ...
        contextMenu
                .getItems()
                .add(factory.createMenuItem(
                        StandardActions.SELECT_ALL, new SearchContextAction(StandardActions.SELECT_ALL)));
        contextMenu
                .getItems()
                .add(factory.createMenuItem(
                        StandardActions.UNSELECT_ALL, new SearchContextAction(StandardActions.UNSELECT_ALL)));
        contextMenu
                .getItems()
                .add(factory.createMenuItem(
                        StandardActions.EXPAND_ALL, new SearchContextAction(StandardActions.EXPAND_ALL)));
        contextMenu
                .getItems()
                .add(factory.createMenuItem(
                        StandardActions.COLLAPSE_ALL, new SearchContextAction(StandardActions.COLLAPSE_ALL)));
