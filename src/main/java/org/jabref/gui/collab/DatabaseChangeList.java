...
public class DatabaseChangeList {
    ...
    public static List<DatabaseChange> compareAndGetChanges(
        ...
        differences
                ...
                .ifPresent(
                        diff -> changes.add(new PreambleChange(diff, originalDatabase, databaseChangeResolverFactory)));
        differences
                .getBibStringDifferences()
                .forEach(diff ->
                        changes.add(createBibStringDiff(originalDatabase, databaseChangeResolverFactory, diff)));
        differences
                .getEntryDifferences()
                .forEach(
                        diff -> changes.add(createBibEntryDiff(originalDatabase, databaseChangeResolverFactory, diff)));
    ...
    private static DatabaseChange createBibStringDiff(
        ...
        if (diff.getOriginalString().getName().equals(diff.getNewString().getName())) {
            return new BibTexStringChange(
                    diff.getOriginalString(), diff.getNewString(), originalDatabase, databaseChangeResolverFactory);
        }
    ...
    private static DatabaseChange createBibEntryDiff(
        ...
        return new EntryChange(
                diff.getOriginalEntry(), diff.getNewEntry(), originalDatabase, databaseChangeResolverFactory);
    }
}
