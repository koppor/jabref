        ...
        differences
                ...
                .ifPresent(
                        diff -> {
                            changes.add(
                                    new MetadataChange(
                                            diff, originalDatabase, databaseChangeResolverFactory));
    ...
    private static DatabaseChange createBibStringDiff(
        ...
        if (diff.getOriginalString() == null) {
            return new BibTexStringAdd(
                    diff.getNewString(), originalDatabase, databaseChangeResolverFactory);
        }

        if (diff.getNewString() == null) {
            return new BibTexStringDelete(
                    diff.getOriginalString(), originalDatabase, databaseChangeResolverFactory);
        }
    ...
    private static DatabaseChange createBibEntryDiff(
        ...
        if (diff.newEntry() == null) {
            return new EntryDelete(
                    diff.originalEntry(), originalDatabase, databaseChangeResolverFactory);
        }
