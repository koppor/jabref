...
public class DatabaseChangeMonitor implements FileUpdateListener {
    ...
    private void notifyOnChange(List<DatabaseChange> changes) {
        // The changes come from {@link
        // org.jabref.gui.collab.DatabaseChangeList.compareAndGetChanges}
        notificationPane.notify(
                ...
                List.of(
                        ...
                        new Action(
                                ...
                                event -> {
                                    DatabaseChangesResolverDialog databaseChangesResolverDialog =
                                            new DatabaseChangesResolverDialog(
                                                    changes,
                                                    database,
                                                    Localization.lang("External Changes Resolver"));
                                    var areAllChangesResolved =
                                            dialogService.showCustomDialogAndWait(
                                                    databaseChangesResolverDialog);
                                    saveState = stateManager.activeTabProperty().get().get();
                                    final NamedCompound ce =
                                            new NamedCompound(
                                                    Localization.lang("Merged external changes"));
                                    changes.stream()
                                    ...
                                    if (areAllChangesResolved.get()) {
                                        if (databaseChangesResolverDialog.areAllChangesAccepted()) {
                                            // In case all changes of the file on disk are merged
                                            // into the current in-memory file, the file on disk
                                            // does not differ from the in-memory file
    ...
    public void fileUpdated() {
        synchronized (database) {
            // File on disk has changed, thus look for notable changes and notify listeners in case
            ...
            BackgroundTask.wrap(scanner::scanForChanges)
                    .onSuccess(
                            changes -> {
                                if (!changes.isEmpty()) {
                                    listeners.forEach(
                                            listener -> listener.databaseChanged(changes));
                                }
                            })
