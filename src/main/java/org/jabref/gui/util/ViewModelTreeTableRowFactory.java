...
public class ViewModelTreeTableRowFactory<S>
    ...
    private final Map<EventType<? extends Event>, BiConsumer<S, ? super Event>> eventFilters =
            new HashMap<>();
    private final Map<PseudoClass, Callback<TreeTableRow<S>, ObservableValue<Boolean>>>
    ...
    public ViewModelTreeTableRowFactory<S> setOnDragDropped(
        ...
        return setOnDragDropped(
                (row, viewModel, event) -> toOnDragDropped.accept(viewModel, event));
    }
    ...
    public ViewModelTreeTableRowFactory<S> setOnMouseDragEntered(
        ...
        return setOnMouseDragEntered(
                (row, viewModel, event) -> toOnDragEntered.accept(viewModel, event));
    }
    ...
    public ViewModelTreeTableRowFactory<S> withEventFilter(
            EventType<? extends Event> event, BiConsumer<S, ? super Event> toCondition) {
                ...
                if (contextMenuFactory != null) {
                    ...
                    treeTableView.addEventHandler(
                            ...
                            event -> {
                                boolean rowFocused =
                                        !isEmpty()
                                                && treeTableView.getFocusModel().getFocusedIndex()
                                                        == getIndex();
                ...
                if (!empty && (row != null)) {
                    ...
                    if (toOnMouseDragEntered != null) {
                        setOnMouseDragEntered(
                                event -> toOnMouseDragEntered.accept(this, getItem(), event));
                    }

                    for (Map.Entry<EventType<?>, BiConsumer<S, ? super Event>> eventFilter :
                            eventFilters.entrySet()) {
                        addEventFilter(
                                eventFilter.getKey(),
                                event -> eventFilter.getValue().accept(getItem(), event));
                    }

                    for (Map.Entry<PseudoClass, Callback<TreeTableRow<S>, ObservableValue<Boolean>>>
