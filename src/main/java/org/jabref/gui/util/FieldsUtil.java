...
public class FieldsUtil {
    ...
    public static final StringConverter<Field> FIELD_STRING_CONVERTER =
            new StringConverter<>() {
                @Override
    ...
    public static String getNameWithType(
            Field field, CliPreferences preferences, UndoManager undoManager) {
        if (field instanceof SpecialField specialField) {
            return new SpecialFieldViewModel(specialField, preferences, undoManager)
                            .getLocalization()
                    + " ("
