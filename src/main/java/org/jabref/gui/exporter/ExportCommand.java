...
public class ExportCommand extends SimpleCommand {
    ...
    public ExportCommand(
        ...
        this.executable.bind(
                exportMethod == ExportMethod.EXPORT_SELECTED
        ...
        FileDialogConfiguration fileDialogConfiguration =
                new FileDialogConfiguration.Builder()
                        ...
                        .withInitialDirectory(
                                preferences.getExportPreferences().getExportWorkingDirectory())
                        .build();
        dialogService
    ...
    private void export(
        ...
        String selectedExtension =
                selectedExtensionFilter.getExtensions().getFirst().replace("*", "");
        if (!file.endsWith(selectedExtension)) {
            FileUtil.addExtension(file, selectedExtension);
        }

        final Exporter format =
                FileFilterConverter.getExporter(selectedExtensionFilter, exporters)
                        .orElseThrow(
                                () ->
                ...
                .onSuccess(
                        save -> {
                            LibraryTab.DatabaseNotification notificationPane =
                                    tabSupplier.get().getNotificationPane();
