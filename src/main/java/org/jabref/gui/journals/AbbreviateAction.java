...
public class AbbreviateAction extends SimpleCommand {
    ...
    private String abbreviate(BibDatabaseContext databaseContext, List<BibEntry> entries) {
        ...
        Set<Callable<Boolean>> tasks =
                entries.stream()
                        .<Callable<Boolean>>map(
                                entry ->
                                        () ->
                                                FieldFactory.getJournalNameFields().stream()
                                                        .anyMatch(
                                                                journalField ->
                                                                        undoableAbbreviator
                                                                                .abbreviate(
    ...
    private String unabbreviate(BibDatabaseContext databaseContext, List<BibEntry> entries) {
        UndoableUnabbreviator undoableAbbreviator =
                new UndoableUnabbreviator(abbreviationRepository);
