...
public class FrameDndHandler {
                    ...
                    scene.get()
                            .setOnDragEntered(
                                    event -> {
                                        ...
                                        // drag'n'drop on tabs covered dnd on tabbedPane, so dnd on
                                        // tabs should contain all dnds on tabbedPane
                                        for (Node destinationTabNode : tabPane.lookupAll(".tab")) {
                                            destinationTabNode.setOnDragOver(
                                                    tabDragEvent ->
                                                            onTabDragOver(
                                                                    event,
                                                                    tabDragEvent,
                                            ...
                                            destinationTabNode.setOnDragDropped(
                                                    tabDragEvent ->
                                                            onTabDragDropped(
                                                                    destinationTabNode,
                                                                    tabDragEvent,
    ...
    private void onTabDragDropped(
            Node destinationTabNode, DragEvent tabDragEvent, Tab dndIndicator) {
        ...
        if (hasBibFiles(dragboard)) {
            ...
            if (hasEntries(dragboard)) {
                List<BibEntry> entryCopies =
                        stateManager.getLocalDragboard().getBibEntries().stream()
    ...
    private void dropGroups(Dragboard dragboard, LibraryTab destinationLibraryTab) {
        ...
        GroupTreeNode destinationLibraryGroupRoot =
                destinationLibraryTab.getBibDatabaseContext().getMetaData().getGroups().get();

        GroupTreeNode groupsTreeNode =
                stateManager.getActiveDatabase().get().getMetaData().getGroups().get();
        ...
        for (String pathToSource : groupPathToSources) {
            ...
            copyGroupTreeNode(
                    destinationLibraryTab, destinationLibraryGroupRoot, groupTreeNodeToCopy);
        }
    }
    ...
    private void copyRootNode(LibraryTab destinationLibraryTab) {
        ...
        GroupTreeNode currentLibraryGroupRoot =
                stateManager.getActiveDatabase().get().getMetaData().getGroups().get().copyNode();
    ...
    private void copyGroupTreeNode(
            LibraryTab destinationLibraryTab,
            GroupTreeNode parent,
            GroupTreeNode groupTreeNodeToCopy) {
        if (stateManager.getActiveDatabase().isEmpty()) {
            return;
        }

        List<BibEntry> allEntries = stateManager.getActiveDatabase().get().getEntries();
        // add groupTreeNodeToCopy to the parent-- in the first run that will the source/main
        // GroupTreeNode
