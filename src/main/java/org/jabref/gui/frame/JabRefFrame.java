...
public class JabRefFrame extends BorderPane implements LibraryTabContainer, UiMessageHandler {
    ...
    public JabRefFrame(
        ...
        this.frameDndHandler =
                new FrameDndHandler(
                        tabbedPane, mainStage::getScene, this::getOpenDatabaseAction, stateManager);
        ...
        this.pushToApplicationCommand =
                new PushToApplicationCommand(
                        stateManager, dialogService, this.preferences, taskExecutor);
    ...
    public void updateDividerPosition() {
        if (mainStage.isShowing() && !sidePane.getChildren().isEmpty()) {
            splitPane.setDividerPositions(
                    preferences.getGuiPreferences().getSidePaneWidth() / splitPane.getWidth());
            dividerSubscription =
                    EasyBind.listen(
                            ...
                            (obs, old, newVal) ->
                                    preferences
                                            .getGuiPreferences()
                                            .setSidePaneWidth(newVal.doubleValue()));
        }
    }
                            ...
                            case PASTE:
                                if (OS.OS_X) { // Workaround for a jdk issue that executes paste
                                    ...
                                    if (!(stateManager.getFocusOwner().isPresent()
                                            && (stateManager.getFocusOwner().get()
                                                    instanceof CodeArea))) {
                                        event.consume();
                                        break;
                                    }
    ...
    private void initBindings() {
        ...
        // This variable cannot be inlined, since otherwise the list created by EasyBind is being
        // garbage collected
        openDatabaseList =
                EasyBind.map(filteredTabs, tab -> ((LibraryTab) tab).getBibDatabaseContext());
        ...
        Platform.runLater(
                () ->
                        stateManager
                                ...
                                .bind(
                                        EasyBind.map(
                                                mainStage.getScene().focusOwnerProperty(),
                                                Optional::ofNullable)));

        EasyBind.subscribe(
                tabbedPane.getSelectionModel().selectedItemProperty(),
                selectedTab -> {
        ...
        EasyBind.subscribe(
                ...
                tab -> {
                    ...
                    if (preferences.getSearchPreferences().shouldKeepSearchString()) {
                        ...
                        stateManager
                                .activeSearchQuery(SearchType.NORMAL_SEARCH)
                                .set(libraryTab.searchQueryProperty().get());
                    }
                    stateManager
                            .searchResultSize(SearchType.NORMAL_SEARCH)
                            .bind(libraryTab.resultSizeProperty());
        ...
        contextMenu
                ...
                .addAll(
                        factory.createMenuItem(
                                ...
                                new LibraryPropertiesAction(
                                        tab::getBibDatabaseContext, stateManager)),
                        factory.createMenuItem(
                        ...
                        factory.createMenuItem(
                                StandardActions.CLOSE_ALL_LIBRARIES, new CloseAllDatabaseAction()));
    ...
    public boolean closeTabs(@NonNull List<LibraryTab> tabs) {
        ...
        List<LibraryTab> toClose =
                tabs.stream().distinct().filter(getLibraryTabs()::contains).toList();
