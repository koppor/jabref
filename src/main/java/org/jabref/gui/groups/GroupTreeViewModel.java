...
public class GroupTreeViewModel extends AbstractViewModel {
    ...
    private final Comparator<GroupTreeNode> compAlphabetIgnoreCase =
            (GroupTreeNode v1, GroupTreeNode v2) -> v1.getName().compareToIgnoreCase(v2.getName());
    private final Comparator<GroupTreeNode> compAlphabetIgnoreCaseReverse =
            (GroupTreeNode v1, GroupTreeNode v2) -> v2.getName().compareToIgnoreCase(v1.getName());
    private final Comparator<GroupTreeNode> compEntries =
            (GroupTreeNode v1, GroupTreeNode v2) -> {
                int numChildren1 =
                        v1.getEntriesInGroup(this.currentDatabase.get().getEntries()).size();
                int numChildren2 =
                        v2.getEntriesInGroup(this.currentDatabase.get().getEntries()).size();
    ...
    private void onActiveDatabaseChanged(Optional<BibDatabaseContext> newDatabase) {
        if (newDatabase.isPresent()) {
            ...
            if (stateManager.getSelectedGroups(newDatabase.get()).isEmpty()) {
                stateManager.setSelectedGroups(
                        newDatabase.get(), Collections.singletonList(newRoot.getGroupNode()));
            }
                ...
                database -> {
                    ...
                    newGroup.ifPresent(
                            group -> {
                                ...
                                // TODO: Add undo
                                // UndoableAddOrRemoveGroup undo = new
                                // UndoableAddOrRemoveGroup(parent, new
                                // GroupTreeNodeViewModel(newGroupNode),
    ...
    public void writeGroupChangesToMetaData() {
        currentDatabase.ifPresent(
                database -> database.getMetaData().setGroups(rootGroup.get().getGroupNode()));
    }
    ...
    boolean onlyMinorChanges(AbstractGroup oldGroup, AbstractGroup newGroup) {
        // we need to use getclass here because we have different subclass inheritance e.g.
        // ExplicitGroup is a subclass of WordKeyWordGroup
        if (oldGroup.getClass() == WordKeywordGroup.class) {
            ...
            return Objects.equals(
                    ...
                    && Objects.equals(
                            oldSearchGroup.getSearchFlags(), newSearchGroup.getSearchFlags());
            ...
            return Objects.equals(
                    oldTexGroup.getFilePath().toString(), newTexGroup.getFilePath().toString());
        }
        return true;
    }
                ...
                database -> {
                    ...
                    newGroup.ifPresent(
                            group -> {
                                ...
                                // We need to check if only the name changed as this is relevant for
                                // the entry's group field
                                if (groupTypeEqual
                                        && !group.getName().equals(oldGroupName)
                                        && onlyMinorModifications) {
                                    int groupsWithSameName = 0;
                                    Optional<GroupTreeNode> databaseRootGroup =
                                            currentDatabase.get().getMetaData().getGroups();
                                    if (databaseRootGroup.isPresent()) {
                                        // we need to check the old name for duplicates. If the new
                                        // group name occurs more than once, it won't matter
                                        groupsWithSameName =
                                                databaseRootGroup
                                                        .get()
                                                        .findChildrenSatisfying(
                                                                g ->
                                                                        g.getName()
                                                                                .equals(
                                                                                        oldGroupName))
                                    ...
                                    dialogService.notify(
                                            Localization.lang(
                                                    "Modified group \"%0\".", group.getName()));
                                    writeGroupChangesToMetaData();
                                    // This is ugly, but we have no proper update mechanism in place
                                    // to propagate the changes, so redraw everything
                                    refresh();
                                    return;
                                }
                                ...
                                if (groupTypeEqual
                                        && onlyMinorChanges(
                                                oldGroup.getGroupNode().getGroup(), group)) {
                                    oldGroup.getGroupNode()
                                ...
                                ButtonType keepAssignments =
                                        new ButtonType(
                                                Localization.lang("Assign"),
                                                ButtonBar.ButtonData.YES);
                                ButtonType removeAssignments =
                                        new ButtonType(
                                ...
                                // UndoableModifyGroup undo = new
                                // UndoableModifyGroup(GroupSelector.this, groupsRoot, node,
                                // newGroup);
    ...
    public void chatWithGroup(GroupNodeViewModel group) {
        ...
        StringProperty nameProperty =
                new SimpleStringProperty(Localization.lang("Group %0", groupNameProperty.get()));
        groupNameProperty.addListener(
                (obs, oldValue, newValue) ->
                        nameProperty.setValue(
                                Localization.lang("Group %0", groupNameProperty.get())));

        ObservableList<ChatMessage> chatHistory =
                aiService
                        .getChatHistoryService()
                        .getChatHistoryForGroup(currentDatabase.get(), group.getGroupNode());
        ObservableList<BibEntry> bibEntries =
                FXCollections.observableArrayList(
                        group.getGroupNode().findMatches(currentDatabase.get().getDatabase()));
    ...
    private void openAiChat(
        ...
        Optional<AiChatWindow> existingWindow =
                stateManager.getAiChatWindows().stream()
        ...
        if (existingWindow.isPresent()) {
            ...
            aiChatWindow.setOnCloseRequest(
                    event -> stateManager.getAiChatWindows().remove(aiChatWindow));
    ...
    public void generateEmbeddings(GroupNodeViewModel groupNode) {
        ...
        List<LinkedFile> linkedFiles =
                currentDatabase.get().getDatabase().getEntries().stream()
        ...
        aiService
                .getIngestionService()
                .ingest(group.nameProperty(), linkedFiles, currentDatabase.get());

        dialogService.notify(
                Localization.lang("Ingestion started for group \"%0\".", group.getName()));
    }
    ...
    public void generateSummaries(GroupNodeViewModel groupNode) {
        ...
        aiService
                .getSummariesService()
                .summarize(group.nameProperty(), entries, currentDatabase.get());

        dialogService.notify(
                Localization.lang("Summarization started for group \"%0\".", group.getName()));
    }
    ...
    public void removeSubgroups(GroupNodeViewModel group) {
        boolean confirmation =
                dialogService.showConfirmationDialogAndWait(
                        ...
                        Localization.lang(
                                "Remove all subgroups of \"%0\"?", group.getDisplayName()));
        if (confirmation) {
            /// TODO: Add undo
            ...
            dialogService.notify(
                    Localization.lang(
                            "Removed all subgroups of group \"%0\".", group.getDisplayName()));
            writeGroupChangesToMetaData();
        }
    }
    ...
    public void removeGroupAndSubgroups(GroupNodeViewModel group) {
        ...
        if (confirmed) {
            ...
            if (selectedGroupNodes.size() > 1) {
                ...
                dialogService.notify(
                        Localization.lang(
                                "Removed group \"%0\" and its subgroups.", group.getDisplayName()));
            }
            writeGroupChangesToMetaData();
        }
    }
    ...
    public void addSelectedEntries(GroupNodeViewModel group) {
        // TODO: Warn
        // if (!WarnAssignmentSideEffects.warnAssignmentSideEffects(node.getNode().getGroup(),
        // panel.frame())) {
        ...
        // if (!undoAdd.isEmpty()) { undo.addEdit(UndoableChangeEntriesOfGroup.getUndoableEdit(node,
        // undoAdd)); }
    ...
    public void removeSelectedEntries(GroupNodeViewModel group) {
        ...
        // if (!WarnAssignmentSideEffects.warnAssignmentSideEffects(mNode.getNode().getGroup(),
        // mPanel.frame())) {
        //    return; // user aborted operation
