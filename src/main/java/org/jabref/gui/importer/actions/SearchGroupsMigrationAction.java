...
public class SearchGroupsMigrationAction implements GUIPostOpenAction {
    ...
    public boolean isActionNecessary(ParserResult parserResult, CliPreferences preferences) {
        if (parserResult.getMetaData().getGroupSearchSyntaxVersion().isPresent()) {
            // Currently the presence of any version is enough to know that no migration is
            // necessary
            return false;
        }
    ...
    public void performAction(
        ...
        if (!dialogService.showConfirmationDialogAndWait(
                ...
                Localization.lang("Migrate"),
                Localization.lang("Keep as is"))) {
            return;
        }
        ...
        parserResult
                .getMetaData()
                .getGroups()
                .ifPresent(groupTreeNode -> migrateGroups(groupTreeNode, dialogService));
    ...
    private void migrateGroups(GroupTreeNode node, DialogService dialogService) {
        if (node.getGroup() instanceof SearchGroup searchGroup) {
            try {
                String luceneSearchExpression =
                        SearchToLuceneMigration.migrateToLuceneSyntax(
                ...
                Optional<String> luceneSearchExpression =
