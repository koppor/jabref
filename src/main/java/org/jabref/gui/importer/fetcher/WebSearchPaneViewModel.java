...
public class WebSearchPaneViewModel {
    ...
    public WebSearchPaneViewModel(
            PreferencesService preferencesService, DialogService dialogService, StateManager stateManager) {
            ...
            try {
                ...
                if (element == null) {
                    ...
                    return ValidationMessage.error(
                            Localization.lang("Invalid query element '%0' at position %1", element, position));
                }
    ...
    public void search() {
        ...
        if (CompositeIdFetcher.containsValidId(query)) {
            CompositeIdFetcher compositeIdFetcher =
                    new CompositeIdFetcher(preferencesService.getImportFormatPreferences());
            parserResultCallable =
                    () -> new ParserResult(OptionalUtil.toList(compositeIdFetcher.performSearchById(query)));
        ...
        Telemetry.getTelemetryClient()
                .ifPresent(client -> client.trackEvent("search", Map.of("fetcher", finalFetcherName), Map.of()));

        BackgroundTask<ParserResult> task =
                BackgroundTask.wrap(parserResultCallable).withInitialMessage(Localization.lang("Processing %0", query));
