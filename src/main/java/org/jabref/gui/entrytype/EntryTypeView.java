...
public class EntryTypeView extends BaseDialog<EntryType> {
    ...
    public EntryTypeView(
            LibraryTab libraryTab, DialogService dialogService, GuiPreferences preferences) {
        ...
        ControlHelper.setAction(
                generateButton, this.getDialogPane(), event -> viewModel.runFetcherWorker());
    ...
    public void initialize() {
        ...
        // we set the managed property so that they will only be rendered when they are visble so
        // that the Nodes only take the space when visible
        ...
        BibDatabaseMode customTypeDatabaseMode =
                isBiblatexMode ? BibDatabaseMode.BIBLATEX : BibDatabaseMode.BIBTEX;
        BibEntryTypesManager entryTypesManager =
                Injector.instantiateModelOrService(BibEntryTypesManager.class);
        List<BibEntryType> customTypes =
                entryTypesManager.getAllCustomTypes(customTypeDatabaseMode);
                ...
                .addListener(
                        (obs, oldValue, newValue) ->
                                visualizer.initVisualization(
                                        viewModel.idFieldValidationStatus(), idTextField, true));
