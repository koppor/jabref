...
class FileDialogConfigurationTest {
    ...
    void withValidDirectoryString(@TempDir Path folder) {
        ...
        FileDialogConfiguration fileDialogConfiguration =
                new FileDialogConfiguration.Builder().withInitialDirectory(tempFolder).build();

        assertEquals(
                Optional.of(Path.of(tempFolder)), fileDialogConfiguration.getInitialDirectory());
    }
    ...
    void withNullStringDirectory() {
        FileDialogConfiguration fileDialogConfiguration =
                new FileDialogConfiguration.Builder().withInitialDirectory((String) null).build();
    ...
    void withNullPathDirectory() {
        FileDialogConfiguration fileDialogConfiguration =
                new FileDialogConfiguration.Builder().withInitialDirectory((Path) null).build();
    ...
    void singleExtension() {
        ...
        FileChooser.ExtensionFilter filter =
                toFilter(
                        "%1s %2s".formatted("BibTex", Localization.lang("Library")),
                        StandardFileType.BIBTEX_DB);

        assertEquals(
                filter.getExtensions(),
                fileDialogConfiguration.getDefaultExtension().getExtensions());
    }

    private FileChooser.ExtensionFilter toFilter(String description, FileType extension) {
        return new FileChooser.ExtensionFilter(
                description,
                extension.getExtensions().stream()
                        .map(ending -> "*." + ending)
