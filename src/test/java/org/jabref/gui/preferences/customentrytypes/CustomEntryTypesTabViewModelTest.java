...
class CustomEntryTypesTabViewModelTest {
    ...
    void storeSettingsKeepsStandardTypes(BibDatabaseMode mode) {
        CustomEntryTypesTabViewModel model =
                new CustomEntryTypesTabViewModel(
                        mode, entryTypesManager, mock(DialogService.class), preferences);
        model.setValues();
        model.storeSettings();
    ...
    void storeSettingsKeepsTypeWhenOrFieldsDiffersOnly() {
        ...
        BibEntryType onlineWithoutOrFields =
                new BibEntryTypeBuilder()
                        ...
                        .build();
        model.entryTypes()
                .setAll(List.of(new CustomEntryTypeViewModel(onlineWithoutOrFields, x -> false)));

        model.storeSettings();

        assertEquals(
                new TreeSet<>(), entryTypesManager.getAllCustomizedTypes(BibDatabaseMode.BIBLATEX));
    }
