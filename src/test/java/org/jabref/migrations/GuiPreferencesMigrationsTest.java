...
class GuiPreferencesMigrationsTest {
    ...
    private final String[] newStylePatterns =
            new String[] {"[citationkey]", "[citationkey] - [title]"};
    ...
    void oldStyleBibtexkeyPattern0() {
        when(preferences.get(JabRefCliPreferences.IMPORT_FILENAMEPATTERN))
    ...
    void arbitraryBibtexkeyPattern() {
        ...
        verify(preferences, never())
                .put(JabRefCliPreferences.IMPORT_FILENAMEPATTERN, arbitraryPattern);
    ...
    void upgradeColumnPreferencesAlreadyMigrated() {
        ...
        when(preferences.getStringList(JabRefGuiPreferences.COLUMN_WIDTHS))
    ...
    void upgradeColumnPreferencesFromWithoutTypes() {
        ...
        List<String> updatedNames =
                Arrays.asList(
                        ...
                        "field:citationkey",
                        "special:printed");
        List<String> updatedWidths =
                Arrays.asList("28", "28", "28", "75", "300", "470", "60", "130", "100", "30");
        List<String> newSortTypes =
                Arrays.asList(
                        "ASCENDING",
                        "ASCENDING",
                        "ASCENDING",
    ...
    void changeColumnPreferencesVariableNamesFor51() {
        ...
        when(preferences.getStringList(JabRefGuiPreferences.COLUMN_NAMES))
                .thenReturn(Collections.emptyList());
        when(preferences.getStringList(JabRefGuiPreferences.COLUMN_WIDTHS))
                .thenReturn(Collections.emptyList());
        when(preferences.getStringList(JabRefGuiPreferences.COLUMN_SORT_TYPES))
                .thenReturn(Collections.emptyList());
        when(preferences.getStringList(JabRefGuiPreferences.COLUMN_SORT_ORDER))
                .thenReturn(Collections.emptyList());
    ...
    void moveApiKeysToKeyRing() throws Exception {
        ...
        when(preferences.getStringList(V5_9_FETCHER_CUSTOM_KEYS))
                .thenReturn(List.of("KeyA", "KeyB", "KeyC"));
        when(preferences.getInternalPreferences().getUserAndHost()).thenReturn("user-host");

        try (MockedStatic<Keyring> keyringFactory =
                Mockito.mockStatic(Keyring.class, Answers.RETURNS_DEEP_STUBS)) {
