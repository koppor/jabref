...
class SearchToLuceneMigrationTest {
    ...
    public static Stream<Arguments> transformationNormal() {
        return Stream.of(
                // If "any:" should not be added, see e46e0a23d7b9526bd449cbecfb189ae6dbc40a28 for a
                // fix
                Arguments.of("any:chocolate", "chocolate"),
                Arguments.of("title:chocolate", "title=chocolate"),
                Arguments.of(
                        "title:chocolate OR author:smith", "title = chocolate or author = smith"),
                Arguments.of("groups:\\/exclude", "groups= /exclude"),
                Arguments.of(
                        "title:chocolate AND author:smith",
                        "title = \"chocolate\" AND author = \"smith\""),
                Arguments.of(
                        "title:chocolate AND author:smith",
                        "title contains \"chocolate\" AND author matches \"smith\""),
                Arguments.of(
                        "( title:chocolate ) OR ( author:smith )",
                        "(title == chocolate) or (author == smith)"),
                Arguments.of(
                        "( title:chocolate OR author:smith ) AND ( year:2024 )",
                        "(title contains chocolate or author matches smith) AND (year = 2024)"),
                ...
                Arguments.of(
                        "( groups:\\:paywall AND -file:/.+/ ) AND -groups:\\/exclude",
                        "groups=:paywall and file!=\"\" and groups!=/exclude"),
                Arguments.of(
                        "( any:* AND -( groups:alpha ) ) AND ( any:* AND -( groups:beta ) )",
                        "NOT(groups=alpha) AND NOT(groups=beta)"),
                // not converted, because not working in JabRef 5.x
                // Arguments.of("title:\"image processing\" OR keywords:\"image processing\"",
                // "title|keywords = \"image processing\""),

                // not converted, because wrong syntax for JabRef 5.x
                // Arguments.of("( author:miller OR title:\"image processing\" OR keywords:\"image
                // processing\" ) AND NOT author:brown AND NOT author:blue", "(author = miller or
                // title|keywords = \"image processing\") and not author = brown and != author =
                // blue"),

                // String with a space
                Arguments.of(
                        "title:image\\ processing AND author:smith",
                        "title = \"image processing\" AND author= smith"),
    ...
    public static Stream<Arguments> transformationRegularExpression() {
        return Stream.of(
                ...
                Arguments.of(
                        "( any:* AND -groups:/.+/ ) AND ( any:* AND -readstatus:/.+/ )",
                        "groups != .+ and readstatus != .+"),
                Arguments.of(
                        "author:/(John|Doe).+(John|Doe)/", "author = \"(John|Doe).+(John|Doe)\""),
