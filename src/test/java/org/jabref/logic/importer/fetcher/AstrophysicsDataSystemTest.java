...
public class AstrophysicsDataSystemTest implements PagedSearchFetcherTest {
    ...
    public void setUp() throws Exception {
        ...
        luceyPaulEntry = new BibEntry(StandardEntryType.Article)
                ...
                .withField(
                        StandardField.KEYWORDS,
                        "Planetology: Solid Surface Planets: Composition, Planetology: Solid Surface Planets: Remote sensing, Planetology: Solid Surface Planets: Surface materials and properties, Planetology: Solar System Objects: Moon (1221), Earth Science")
    ...
    public void searchByEntryFindsEntry() throws Exception {
        BibEntry searchEntry =
                new BibEntry().withField(StandardField.TITLE, "slice theorem").withField(StandardField.AUTHOR, "Diez");

        List<BibEntry> fetchedEntries = fetcher.performSearch(searchEntry);

        // The list contains more than one element, thus we need to check in two steps and cannot use
        // assertEquals(List.of(diezSliceTheoremEntry, fetchedEntries))
        assertFalse(fetchedEntries.isEmpty());
        assertTrue(fetchedEntries.contains(diezSliceTheoremEntry));
    }
