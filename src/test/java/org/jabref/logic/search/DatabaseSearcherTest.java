...
public class DatabaseSearcherTest {
    ...
    void setUp() {
        ...
        when(filePreferences.fulltextIndexLinkedFilesProperty())
                .thenReturn(mock(BooleanProperty.class));
    ...
    @ParameterizedTest
    @MethodSource
    void testDatabaseSearcher(
            List<BibEntry> expectedMatches, SearchQuery query, List<BibEntry> entries)
            throws IOException {
        for (BibEntry entry : entries) {
            databaseContext.getDatabase().insertEntry(entry);
        }
        List<BibEntry> matches =
                new DatabaseSearcher(query, databaseContext, TASK_EXECUTOR, filePreferences)
                        .getMatches();
    ...
    private static Stream<Arguments> testDatabaseSearcher() {
        ...
        return Stream.of(
                Arguments.of(
                        List.of(),
                        new SearchQuery("whatever", EnumSet.noneOf(SearchFlags.class)),
                        List.of()),
                Arguments.of(
                        List.of(),
                        new SearchQuery("whatever", EnumSet.noneOf(SearchFlags.class)),
                ...
                Arguments.of(
                        List.of(),
                        new SearchQuery("author:", EnumSet.noneOf(SearchFlags.class)),
                        List.of(articleEntry)),
                Arguments.of(
                        List.of(articleEntry),
                        new SearchQuery("harrer", EnumSet.noneOf(SearchFlags.class)),
                        List.of(articleEntry)),
                Arguments.of(
                        List.of(),
                        new SearchQuery("title: harrer", EnumSet.noneOf(SearchFlags.class)),
                        List.of(articleEntry)),
                Arguments.of(
                        List.of(inCollectionEntry),
                        new SearchQuery("tonho", EnumSet.noneOf(SearchFlags.class)),
                        List.of(inCollectionEntry)),
                Arguments.of(
                        List.of(inCollectionEntry),
                        new SearchQuery("tonho", EnumSet.noneOf(SearchFlags.class)),
