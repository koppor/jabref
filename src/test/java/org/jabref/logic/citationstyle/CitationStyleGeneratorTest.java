...
class CitationStyleGeneratorTest {
    ...
    void testIgnoreNewLine() {
        ...
        String expected = "  <div class=\"csl-entry\">\n"
                ...
                + "  </div>\n";
        String citation =
                CitationStyleGenerator.generateCitation(entry, CitationStyle.getDefault(), bibEntryTypesManager);
        assertEquals(expected, citation);
    }
    ...
    void testMissingCitationStyle() {
        ...
        String citation =
                CitationStyleGenerator.generateCitation(new BibEntry(), "faulty citation style", bibEntryTypesManager);
        assertEquals(expected, citation);
    }
    ...
    void testHtmlFormat() {
        ...
        String actualCitation = CitationStyleGenerator.generateCitation(
                entry, style, format, new BibDatabaseContext(), bibEntryTypesManager);
    ...
    void testTextFormat() {
        ...
        String actualCitation = CitationStyleGenerator.generateCitation(
                entry, style, format, new BibDatabaseContext(new BibDatabase(List.of(entry))), bibEntryTypesManager);
        assertEquals(expectedCitation, actualCitation);
    }
    ...
    void testHandleCrossRefFields() {
        ...
        String actualCitation = CitationStyleGenerator.generateCitation(
                firstEntry, style, CitationStyleOutputFormat.TEXT, bibDatabaseContext, bibEntryTypesManager);
        assertEquals(expectedCitation, actualCitation);
    }
    ...
    static Stream<Arguments> testCslMapping() {
        ...
        return Stream.of(
                ...
                // The issue field does not exist in bibtex standard, therefore there is no need to render it (it exists
                // in biblatex standard though). Since, for this entry, there is no number field present and therefore
                // no data will be overwriten, enabling the user to be able to move the data within the issue field to
                // the number field via cleanup action is something worth pursuing.
                Arguments.of(
                ...
                // "Article number" is not a field that exists in Bibtex standard. Printing the article number in the
                // pages field is a workaround. Some Journals have opted to put the article number into the pages field.
                // APA 7th Style recommends following procedure: If the journal article has an article number instead of
                // a page range, include the word "Article" and then the article number instead of the page range.
                // Question: Should it be rendered WITH Article or WITHOUT the Word Article in Front? I guess without?
                Arguments.of(
                ...
                // "The issue field does not exist in bibtex standard, therefore there is no need to render it (it
                // exists in biblatex standard though). Since, for this entry, there is no number field present and
                // therefore no data will be overwriten, enabling the user to be able to move the data within the issue
                // field to the number field via cleanup action is something worth pursuing."
                Arguments.of(
                ...
                // Not rendering the "eid" field here, is correct. The "eid" field(short for: electronic identifier)
                // does not exist in Bibtex standard. It exists in Biblatex standard though and is the field, in which
                // the "article number" should be entered into. "Article number" is a field that does not exist in
                // Bibtex and also not in Biblatex standard. As a workaround, some Journals have opted to put the
                // article number into the pages field. APA 7th Style recommends following procedure: "If the journal
                // article has an article number instead of a page range, include the word "Article" and then the
                // article number instead of the page range." - Source:
                // https://apastyle.apa.org/style-grammar-guidelines/references/examples/journal-article-references#2.
                // Additionally the APA style (7th edition) created by the CSL community "prints the issue (= the
                // "number" field" in Biblatex)  whenever it is in the data and the number (= the "eid" field in
                // Biblatex) when no page range is present, entirely independent of the issue number" - Source:
                // https://github.com/citation-style-language/styles/issues/5827#issuecomment-1006011280). I personally
                // think the "eid" field SHOULD be rendered here SOMEWHERE, maybe even IN ADDITION to the page range,
                // because we have the data, right? Why not show it? - But this is just my humble opinion and may not be
                // coherent with the current APA Style 7th edition. Not rendering the "issue" field here is sufficient
                // for APA 7th edition. Under current circumstances the "number" field takes priority over the "issue"
                // field (see https://github.com/JabRef/jabref/issues/8372#issuecomment-1023768144). [Keyword: IS
                // RENDERING BOTH VIABLE?]. Ideally, they would coexist: "Roughly speaking number subdivides volume and
                // issue is much closer to subdividing year. I don't think I would want to say that issue is subordinate
                // to number or vice versa. They sort of operate on a similar level." (Source:
                ...
                // Not rendering the "issue" field here is sufficient for APA Style (7th edition). Under current
                // circumstances the "number" field takes priority over the "issue" field (see
                // https://github.com/JabRef/jabref/issues/8372#issuecomment-1023768144). [Keyword: IS RENDERING BOTH
                // VIABLE?]. Ideally, they would coexist: "Roughly speaking number subdivides volume and issue is much
                // closer to subdividing year. I don't think I would want to say that issue is subordinate to number or
                // vice versa. They sort of operate on a similar level." (Source:
                // https://github.com/plk/biblatex/issues/726#issuecomment-1010264258)
                Arguments.of(
                        "Foo, B. (n.d.). issue + number. Bib(La)TeX Journal, 3number.\n",
                        BibDatabaseMode.BIBLATEX,
                ...
                // Not rendering the "issue" field here is sufficient for APA 7th edition. Under current circumstances
                // the "number" field takes priority over the "issue" field (see
                // https://github.com/JabRef/jabref/issues/8372#issuecomment-1023768144). [Keyword: IS RENDERING BOTH
                // VIABLE?]. Ideally, they would coexist: "Roughly speaking number subdivides volume and issue is much
                // closer to subdividing year. I don't think I would want to say that issue is subordinate to number or
                // vice versa. They sort of operate on a similar level." (Source:
