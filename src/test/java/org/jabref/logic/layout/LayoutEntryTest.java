...
public class LayoutEntryTest {
    ...
    void setUp() {
        ...
        //  Specifically, we present a core typed language to describe Android applications, and to
        // reason about their data-flow security properties. Our operational semantics and type
        // system provide some necessary foundations to help both users and developers of Android
        // applications deal with their security concerns.
        ...
        mBTE.setField(
                new UnknownField("citeulike-linkout-0"),
                "http://portal.acm.org/citation.cfm?id=1554339.1554341");
    ...
    void parseMethodCalls() {
        ...
        assertEquals(
                "_bla.bla.blub",
                (LayoutEntry.parseMethodsCalls("_bla.bla.blub,").getFirst()).getFirst());
        ...
        assertEquals(
                "bla",
                (LayoutEntry.parseMethodsCalls("bla(\"test\"),foo(\"fark\")").getFirst())
                        .getFirst());
        assertEquals(
                "foo",
                (LayoutEntry.parseMethodsCalls("bla(\"test\"),foo(\"fark\")").get(1)).getFirst());
        assertEquals(
                "test",
                (LayoutEntry.parseMethodsCalls("bla(\"test\"),foo(\"fark\")").getFirst()).get(1));
        assertEquals(
                "fark",
        ...
        assertEquals(
                "bla",
                (LayoutEntry.parseMethodsCalls("bla(test),foo(fark)").getFirst()).getFirst());
        assertEquals(
                "foo", (LayoutEntry.parseMethodsCalls("bla(test),foo(fark)").get(1)).getFirst());
        assertEquals(
                "test", (LayoutEntry.parseMethodsCalls("bla(test),foo(fark)").getFirst()).get(1));
