...
class LatexIntegrityCheckerTest {
    ...
    private static Stream<Arguments> provideAcceptedInputs() {
        return Stream.of(
                ...
                // Arguments.of(StandardField.TITLE, "\\begin{align} x + y &= z \\\\ a &= b + c
                // \\end{align}"),
                // Arguments.of(StandardField.TITLE, "\\begin{align*} A &:= B \\\\ C &\\rightarrow D
                // \\end{align*}"),
                // Arguments.of(StandardField.TITLE, "\\begin{equation} E = mc^2 \\end{equation}"),
                // Arguments.of(StandardField.TITLE, "\\begin{align*} x + y &= z \\\\ a &= b + c
                // \\\\ p &= \\frac{q}{r} \\end{align*}"),
                Arguments.of(
                        StandardField.TITLE,
                        "\\begin{eqnarray} x + y &= z \\\\ a &= b + c \\\\ p &= \\frac{q}{r} \\end{eqnarray}"),
                ...
                Arguments.of(
                        StandardField.TITLE,
                        "\\[ \\begin{bmatrix} x & y & z \\\\ u & v & w \\end{bmatrix} \\]"),

                // Tables
                Arguments.of(
                        StandardField.TITLE,
                        "\\begin{tabular}{|c|c|} \\hline 1 & 2 \\\\ 3 & 4 \\\\ \\hline \\end{tabular}"),
                Arguments.of(
                        StandardField.TITLE,
                        "\\begin{tabular}{cc} A & B \\\\ C & D \\end{tabular}"),
                Arguments.of(
                        StandardField.TITLE,
                        "\\begin{tabular}{|l|r|} \\hline Item & Quantity \\\\ \\hline Apple & 3 \\\\ Banana & 5 \\\\ \\hline \\end{tabular}"),
                ...
                Arguments.of(
                        StandardField.TITLE, "\\textbf{\\emph{Bold and Emphasized Text}} $5-3_k$"),
                Arguments.of(
                        StandardField.TITLE,
                ...
                // Arguments.of(StandardField.TITLE, "\\begin{figure} \\centering
                // \\includegraphics[width=0.8\\textwidth]{plot.png} \\caption{Plot Caption}
                // \\label{fig:plot} \\end{figure}"),
                ...
                Arguments.of(
                        ...
                        "\\textless{}xml\\textgreater{} \\textbar something \textbackslash"),

                // Mirrored from
                // org.jabref.logic.integrity.AmpersandCheckerTest.provideAcceptedInputs
                ...
                Arguments.of(
                        ...
                        "_something_ which is triggers the integrity check ^^^ $"));
    }
    ...
    void rejectsDisallowedInputs(String expectedMessage, Field field, String value) {
        entry.setField(field, value);
        assertEquals(
                List.of(new IntegrityMessage(expectedMessage, entry, field)), checker.check(entry));
    }
    ...
    private static Stream<Arguments> provideUnacceptedInputs() {
        return Stream.of(
                ...
                // TTEG02: Non-ASCII character {0} (Unicode U+{1}) at offset {2} in input document -
                // replaced with ’x’
                // todo find out how to trigger / activate - is this even implemented in snuggletex?
                // Arguments.of(LatexIntegrityChecker.errorMessageFormatHelper(CoreErrorCode.TTEG02,
                // "€", "20AC", "3"), StandardField.TITLE, "A title with € symbol"),
                ...
                // TTEG04: Argument placeholder tokens (e.g. #1) may only appear in command and
                ...
                // TTEM03: Math superscript (^) and subscript (_) characters are not allowed in text
                // mode
                Arguments.of(
                ...
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEC00, "undefinedCommand"),
                        StandardField.ABSTRACT,
                        "\\undefinedCommand"),
                ...
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEE00, "align", "itemize"),
                        StandardField.ABSTRACT,
                        "\\begin{itemize} \\end{align}"),

                // TTEE01: Expected to read valid environment name enclosed in braces without
                // whitespace
                Arguments.of(
                ...
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEE02, "undefinedEnv"),
                        StandardField.ABSTRACT,
                        "\\begin{undefinedEnv}"),
                ...
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEE06, "tabular", "1"),
                        StandardField.TITLE,
                        "\\begin{tabular}\\end{tabular}"),

                // TTEU00: Expansion limit ({0}) for user-defined commands and environments has been
                // exceeded. Possible recursion?
                // Skipped
                ...
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEUC3, "MyCommand"),
                        StandardField.TITLE,
                        "\\newcommand{\\MyCommand}"),
                ...
                // TTEUC7: Number of arguments specified in command or environment definition {0}
                // must be an integer between 1 and 9 - not {1}
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEUC7, "MyCommand", "0"),
                        StandardField.TITLE,
                ...
                // TTEUCA: Definition of command {0} refers to argument #{1} but only {2} have been
                // declared
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEUCA, "MyCommand", "2", "1"),
                        StandardField.TITLE,
                ...
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEUE1, "begin", "MyEnvironment"),
                        StandardField.TITLE,
                ...
                // TTEUE5: Definition of begin of environment {0} refers to argument #{1} but only
                // {2} have been declared
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEUE5, "MyEnvironment", "1", "0"),
                        StandardField.TITLE,
                        "\\newenvironment{MyEnvironment}{#1}{}"),

                // TTEUE6: Definition of end of environment {0} refers to argument #{1} but
                // arguments may not be used here
                Arguments.of(
                        LatexIntegrityChecker.errorMessageFormatHelper(
                                CoreErrorCode.TTEUE6, "MyEnvironment", "2"),
                        StandardField.TITLE,
