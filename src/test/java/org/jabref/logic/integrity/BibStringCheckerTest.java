...
class BibStringCheckerTest {
    ...
    private static Stream<Arguments> provideAcceptedInputs() {
        return Stream.of(
                Arguments.of(
                        Collections.emptyList(), StandardField.TITLE, "Not a single hash mark"),
                Arguments.of(Collections.emptyList(), StandardField.MONTH, "#jan#"),
                Arguments.of(
                        Collections.emptyList(), StandardField.AUTHOR, "#einstein# and #newton#"));
    }
        ...
        assertEquals(
                List.of(
                        new IntegrityMessage(
                                "odd number of unescaped '#'", entry, StandardField.MONTH)),
                checker.check(entry));
    }
    ...
    void authorDoesNotAcceptOddNumberOfHashMarks() {
        ...
        assertEquals(
                List.of(
                        new IntegrityMessage(
                                "odd number of unescaped '#'", entry, StandardField.AUTHOR)),
                checker.check(entry));
    }
}
